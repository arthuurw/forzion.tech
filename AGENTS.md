...
- `specification-design-review.md` — checklist PRE-código, gêmeo do coding; rodar ANTES de codar feature que toca auth/webhook/concorrência/PII/pagamento. Pega a CLASSE latente (race, defense-in-depth, abuso, PII em repouso) que teste+review não veem. Saída vai no `design.md` da feature.
...
- `specification-load-testing.md` — harness de medição empírica (`scripts/perf/`), REPORT-ONLY. DB pesado (seed/EXPLAIN/pgbench) roda LOCAL/efêmero ou em projeto Supabase de TESTE dedicado — **NUNCA em prod nem em homolog** (recurso compartilhado, cap de storage). Lighthouse pode rodar em homolog (read-only). GOTCHA: `npm run analyze` é no-op sob Turbopack → `next experimental-analyze`. Bench local ≠ Supabase Free: o sinal é o DELTA, não o ms absoluto.
...
- `specification-frontend-ui.md` — WCAG 2.1 AA é ALVO declarado, sem auditoria formal; cobertura de a11y desigual (componentes de `forms/` sem teste). F18 (color-contrast) resolvido.
...
8. BUDGET DE CONTEXTO: principal com 2 faróis — **LARANJA em ~400k** (começar a delegar leitura volumosa e a enxugar o que já está em contexto) e **VERMELHO em ~600k** (rodar `/compact`, salvo ordem explícita em contrário). Abaixo de 400k, trabalhar sem cerimônia. Delegar leitura/implementação volumosa a sub-agents (sumário só — regra 7); não reler o que já está em contexto; ler trechos (offset/limit); manter no principal só orquestração/decisão/integração.
