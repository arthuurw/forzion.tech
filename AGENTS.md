...
8. Criar/atualizar qualquer arquivo `README` deve seguir os PRINCÍPIOS da skill `docs-writer`: precisão ancorada no código, voz ativa, consistência de termos, verificação de links e ler a versão atual antes de editar. A skill em si é referência opcional (foi desenhada p/ outra estrutura de repo — `packages/`, `docs/sidebar.json` etc., que não existem aqui); o obrigatório são os princípios, não a cerimônia.
