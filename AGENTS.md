...
Toda alteração de código DEVE, antes do PR: (1) build completo (frontend + backend) — ATENÇÃO: `forzion.tech.slnx` NÃO inclui `forzion.tech.PactVerification`; ao mexer em Domain (factories `Criar`/`Result<T>`/assinaturas) ou em responses/handlers, buildar esse projeto à parte (`dotnet build forzion.tech.PactVerification/forzion.tech.PactVerification.csproj`), senão quebra SÓ no push em homolog (ver `specs/specification-local-ci-repro.md` §2); (2) avaliar necessidade de novos testes e criá-los se faltarem; (3) rodar TODOS os testes — confirmar que nada quebrou e se algo precisa ser complementado (integração/E2E exigem Docker; sem Docker local, o CI os roda no PR); (4) confirmar PR → `homolog` (ou → `master` se a alteração foi feita direto na branch `homolog`).
