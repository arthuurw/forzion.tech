...
11. SEM MEMORY.md: conhecimento durável NÃO mora em memory local (não-versionada, apodrece sem gate, recall infla ~50% falso-positivo). Roteia por TIPO: regra de trabalho/feedback → ESTE AGENTS.md; fato de domínio/infra/gotcha → `specification-*` da área (atualizar na MESMA task — DoD#9a); backlog de feature especificada-não-feita + estado transitório (PR pendente, segredo a setar, fase parcial) → card no board (`specification-workflow`, Fluxo A). Obra concluída → NÃO registrar (git é o registro). MEMORY.md está aposentada — não escrever nela.
