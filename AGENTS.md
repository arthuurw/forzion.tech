...
8. BUDGET DE CONTEXTO: a janela de contexto do agente PRINCIPAL não deve ultrapassar ~160-170k tokens. Monitorar sempre. Estratégias: delegar leitura/exploração/implementação volumosa a sub-agents (que devolvem só sumário — ver regra 7); evitar reler arquivos já em contexto; ler apenas trechos necessários (offset/limit) em vez de arquivos inteiros; manter no principal só orquestração, decisões e integração. Sempre que possível, preferir sub-agents a inflar o contexto principal.
