...
public class DefinirDadosFiscaisTreinadorHandlerTests
{
    ...
    [Fact]
    public void Handler_NaoInjetaRepositorioDeNotaNemOutbox()
    {
        var tiposDeParametros = typeof(DefinirDadosFiscaisTreinadorHandler)
            .GetConstructors().Single().GetParameters()
            .Select(p => p.ParameterType.Name)
            .ToList();

        tiposDeParametros.Should().NotContain("INotaFiscalRepository");
        tiposDeParametros.Should().NotContain("IOutboxEnfileirador");
    }
}
