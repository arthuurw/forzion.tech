...
public class CriarExercicioHandlerTests
{
    ...
    public async Task HandleAsync_DadosValidos_CadastraERetorna()
    {
        ...
        var command = new CriarExercicioCommand(treinadorId, "Supino Reto", forzion.tech.Domain.Enums.TipoGrupoMuscular.Peito, "Descrição");

        var result = await _handler.HandleAsync(command);

        result.Value.Nome.Should().Be("Supino Reto");
        result.Value.GrupoMuscular.Should().Be(forzion.tech.Domain.Enums.TipoGrupoMuscular.Peito);
    ...
    {
        var command = new CriarExercicioCommand(Guid.NewGuid(), "", forzion.tech.Domain.Enums.TipoGrupoMuscular.Peito, new string('a', 501));
