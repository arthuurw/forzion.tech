...
public class AssinaturaCriadaEmailHandlerTests
{
    private readonly Mock<IAlunoRepository> _alunoRepo = new();
    private readonly Mock<ITreinadorRepository> _treinadorRepo = new();
    private readonly Mock<IPacoteAlunoRepository> _pacoteRepo = new();
    private readonly Mock<IEmailService> _emailService = new();
    private readonly Mock<ILogger<AssinaturaCriadaEmailHandler>> _logger = new();
    private readonly AssinaturaCriadaEmailHandler _handler;

    private static readonly AssinaturaCriadaEvent Evento = new(
        Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), 250.00m, DateTime.UtcNow);

    public AssinaturaCriadaEmailHandlerTests()
    {
        ...
        _handler = new AssinaturaCriadaEmailHandler(
    ...
    [Fact]
    public async Task HandleAsync_HappyPath_EnviaEmailComDadosDaAssinatura()
    {
        var aluno = Aluno.Criar(Guid.NewGuid(), "João", email: "joao@example.com");
        var treinador = Treinador.Criar(Guid.NewGuid(), "Lucas");
        var pacote = PacoteAluno.Criar(Guid.NewGuid(), "Pacote Premium", 250.00m);

        _alunoRepo.Setup(r => r.ObterPorIdAsync(Evento.AlunoId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(aluno);
        _treinadorRepo.Setup(r => r.ObterPorIdAsync(Evento.TreinadorId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(treinador);
        _pacoteRepo.Setup(r => r.ObterPorIdAsync(Evento.PacoteAlunoId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(pacote);
    ...
    public async Task HandleAsync_TreinadorNaoEncontrado_UsaFallback()
    {
        ...
        var pacote = PacoteAluno.Criar(Guid.NewGuid(), "Básico", 150.00m);
