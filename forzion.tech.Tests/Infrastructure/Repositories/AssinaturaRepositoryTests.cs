...
public class AssinaturaRepositoryTests(InfrastructureTestFixture fixture)
{
    private static AssinaturaRepository Repo(AppDbContext ctx) => new(ctx);
    ...
    private static async Task<SeedCtx> SeedContextAsync(AppDbContext ctx, Guid? existingAlunoId = null)
    {
        ...
        var pacote = PacoteAluno.Criar(treinador.Id, $"Pac{Guid.NewGuid():N}", 99.90m);
    ...
    private static async Task<Assinatura> SeedAssinaturaAsync(
        AppDbContext ctx,
        SeedCtx seed,
        bool ativa = true,
        bool cancelada = false)
    {
        var a = Assinatura.Criar(seed.VinculoId, seed.PacoteId, seed.TreinadorId, seed.AlunoId, 99.90m);
        if (ativa) a.Ativar();
        if (cancelada) a.Cancelar();
        await ctx.Assinaturas.AddAsync(a);
        await ctx.SaveChangesAsync();
        return a;
    }
    ...
    public async Task ObterPorIdAsync_Existe_Retorna()
    {
        ...
        var assinatura = await SeedAssinaturaAsync(ctx, seed);
    ...
    public async Task ListarParaRenovarAsync_AtivaComCobrancaPassada_Retorna()
    {
        ...
        var assinatura = await SeedAssinaturaAsync(ctx, seed, ativa: true);
    ...
    public async Task ListarParaRenovarAsync_StatusCancelada_NaoRetorna()
    {
        ...
        var assinatura = await SeedAssinaturaAsync(ctx, seed, ativa: true, cancelada: true);
    ...
    public async Task ListarParaRenovarAsync_StatusPendente_NaoRetorna()
    {
        ...
        var assinatura = await SeedAssinaturaAsync(ctx, seed, ativa: false);
    ...
    [Fact]
    public async Task ListarPorAlunoAsync_MultiplaAssinaturas_OrdenaPorCreatedAtDesc()
    {
        await using var ctx = fixture.CreateContext();
        ...
        var a2 = await SeedAssinaturaAsync(ctx, seed2);
