...
describe("AgendaTreinadorPage", () => {
  ...
  it("abre direto na aba Solicitações quando a URL traz ?tab=solicitacoes", async () => {
    query = "tab=solicitacoes";
    server.use(
      http.get("*/treinador/agenda/bloqueios", () => HttpResponse.json([])),
      http.get("*/treinador/agenda/politica", () => HttpResponse.json(POLITICA_PADRAO)),
      http.get("*/treinador/agenda/solicitacoes", () =>
  ...
  it("sem o parâmetro de aba, abre na aba Bloqueios (comportamento atual preservado)", async () => {
    server.use(
      http.get("*/treinador/agenda/bloqueios", () => HttpResponse.json([])),
      http.get("*/treinador/agenda/politica", () => HttpResponse.json(POLITICA_PADRAO)),
    );
    ...
    await screen.findByText(/nenhum bloqueio pontual cadastrado/i);
    expect(screen.getByRole("tab", { name: /^bloqueios$/i })).toHaveAttribute("aria-selected", "true");
  });
});
