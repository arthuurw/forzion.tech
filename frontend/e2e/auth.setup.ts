 ...
 *
 * Se uma variavel falta, o teste eh skip-ed (nao falha o build). Esse pattern
 * permite que CI rode mesmo sem creds (Fase 9 = infra; Fase 10 valida com creds
 * reais em homologacao).
 */
...
for (const { role, emailVar, passwordVar } of ROLES) {
  setup(`autentica role ${role}`, async ({ request }) => {
    ...
    setup.skip(
      !email || !password,
      `defina ${emailVar} e ${passwordVar} pra gerar storage state de ${role}`,
    );
    ...
    expect(response.ok(), `${role}: login falhou (${response.status()})`).toBeTruthy();
