...
- Backend: logging estruturado via `ILogger` + sink ERROR→DB + sink ERROR→Sentry (ambos `>=Error`, coexistem, nenhum substitui o outro — T4/#375, 2026-08-08); `/health` liveness + `/health/ready` readiness (DbContextCheck + SchemaHealthCheck + StripeHealthCheck + ResendHealthCheck + WhatsAppHealthCheck); relatório de saúde diário por e-mail; alert de chargeback via LogCritical (agora chega no Sentry, ver §1/§6); auth-failure LogWarning (R1-R4, **abaixo do teto `>=Error` dos dois sinks — segue só no stdout**, com 1 exceção: reuso de refresh token tem captura explícita desde 2026-08-09, ver §1 "Sinal de segurança explícito"); auditoria durável de ações privilegiadas (R10-R14, `logs_aprovacao`). **Sentry backend**: `Sentry.Extensions.Logging` (`AddSentry` no `ILoggingBuilder`, NÃO `Sentry.AspNetCore` — sem middleware, sem captura automática de request/user, menor superfície de PII), `Sentry:Dsn` ausente = no-op + `LogWarning` no boot (mesmo padrão `NullEmailService`, nunca fail-fast — Sentry é observabilidade, não caminho de negócio). Projeto Sentry **compartilhado** com o frontend (mesma org, decisão 2026-08-09 — ver §1).
...
- `Sentry:Dsn` (config, override `Sentry__Dsn` nos compose — mesma convenção `Resend__ApiKey`) ausente/em branco/**sintaticamente inválido** → `AddSentryLogging` retorna sem registrar nada (no-op); `Program.cs` loga 1 `LogWarning` no boot fora de env `Test`. Nunca fail-fast — diferente do e-mail (`ResendEmailService`, que aborta boot em prod sem `Resend:ApiKey`): Sentry é observabilidade pura, elevar a disponibilidade da app à de um serviço terceiro só de telemetria não se justifica.
  - **GOTCHA (achado pelo Verifier)**: `Sentry.Dsn.Parse` lança `UriFormatException` pra string malformada — sem guard, isso estoura na 1ª resolução do `ILoggerProvider` (durante o boot do host, NÃO lazily no 1º log; a garantia de "SDK nunca derruba a app" cobre falha de ENTREGA pós-init, não falha de PARSE do DSN). `DsnValido` (`DependencyInjectionExtensions.cs`) valida `Uri.TryCreate` absoluto + scheme http/s + `UserInfo` presente ANTES de chamar `AddSentry` — mesmo guard reusado no warning de boot do `Program.cs`. Um `Sentry:Dsn` digitado errado no `.env` da VM cai no mesmo no-op do DSN ausente, não derruba o deploy.
...
- `SetBeforeSend` → `ScrubPii` (`DependencyInjectionExtensions.cs`, `internal`, testado direto via `InternalsVisibleTo`): roda `MascaraPii.Scrub` em `@event.Message.Formatted`, em cada `SentryException.Value` (`Sentry.Protocol.SentryException`) e em cada valor de `@event.Tags` ANTES do evento sair do processo — mesma função usada pelo sink de DB e pelo `HealthReportCollector.Scrub` (§ acima), não uma cópia. `Message.Message` (template cru, ex. `"falha para {Email}"`) é preservado sem scrub — é só o placeholder, não o valor interpolado; perdê-lo quebraria o agrupamento de issues por template no Sentry.
  - **GOTCHA (achado no code-review, verificado empiricamente)**: `Sentry.Extensions.Logging` grava argumento estruturado de `ILogger` (`LogError("... {Email} ...", email)`) direto em `SentryEvent.Tags[chave]=valorCru` — CAMINHO SEPARADO de `Message`/exception, fora do scrub original. Um call-site que loga PII como argumento estruturado (em vez de já vir mascarado, como os call-sites atuais de `ResendEmailService`/`MetaWhatsAppCloudNotifier` fazem por convenção) vazava PII pro Sentry apesar do `ScrubPii`. `DsnValido` também sub-validava: só checava scheme+userinfo, não o project-id no path nem a public key antes do `:` — 3 formas de DSN malformado (sem project-id, path vazio, public key vazia) ainda derrubavam `SentrySdk.Init` mesmo passando no guard. Ambos corrigidos no mesmo diff, verificados contra `Sentry`/`Sentry.Extensions.Logging` 6.8.0 via scratch project (não só leitura de doc).
- Projeto Sentry **COMPARTILHADO** com o frontend (mesma org, `o4511870158176256` — decisão 2026-08-09, `secprod-observabilidade`): trade-off aceito conscientemente — zero setup novo vs. misturar volume/alerta/retenção de erro client-side com sinal de segurança server-side. Revertido do plano original (projeto dedicado); se o volume do frontend afogar o sinal do backend no futuro, separar é reversível (criar projeto novo + trocar `BACKEND_SENTRY_DSN`).
...
### Sinal de segurança explícito (`IAlertaSegurancaSentry`, `secprod-observabilidade`, 2026-08-09)
- **Motivação**: `MinimumEventLevel = LogLevel.Error` (acima) filtra fora QUALQUER `LogWarning`, incluindo os 2 únicos sinais de credencial comprometida/abuso que o sistema tem (reuse de refresh token, rejeição de rate-limit `auth`/`mfa` — [specification-security] §4). Baixar o teto global pra `Warning` inundaria o Sentry com todo `LogWarning` do app (diluição, custo de quota). Solução: captura EXPLÍCITA, fora do pipeline de `ILogger`, só pros 2 sinais.
- **Contrato**: `IAlertaSegurancaSentry.Registrar(string sinal, string mensagem, IReadOnlyDictionary<string,string> tags)` (`Application/Interfaces`). Impl real `SentrySecurityAlertSink` (`Infrastructure/Logging`) chama `SentrySdk.CaptureMessage(mensagem, scope => { scope.Level = SentryLevel.Warning; scope.SetTag("security_signal", sinal); ...tags })` — API pública confirmada via context7 (`getsentry/sentry-docs`). Impl no-op `AlertaSegurancaSentryNulo` é o default incondicional (registrado ANTES do guard `!IsEnvironment("Test")`, pra `RefreshTokenService`/fixtures E2E que chamam `AddInfrastructure` fora desse guard não quebrarem a resolução de DI); `SentrySecurityAlertSink` sobrescreve só quando `DsnValido(dsn)` (mesma guarda de `AddSentryLogging` — `GetRequiredService<T>` resolve o último registro).
- **Call sites**: `RefreshTokenService.Reuse` → `Registrar("refresh_reuse", ..., {FamiliaId, ContaId})`; `RegistrarRejeicaoAuth` (`DependencyInjectionExtensions.cs`, `OnRejected` do RateLimiter) → `Registrar("rate_limit_rejection", ..., {Politica, Rota, Ip})`, pras políticas `auth`/`mfa`/`agents` (`agents` incluída em T36, auditoria 2026-08-26 — resposta 429 ao chamador continua um `ProblemDetails` avulso fora do envelope `AgentProblem`, [specification-security] §4, mas o sinal de alerta server-side é o MESMO call site das outras políticas).
- **PII**: `CaptureMessage`/`SetTag` passam pelo MESMO hub/`SetBeforeSend`→`ScrubPii` do sink de `ILogger` (o hook é do client Sentry, não da integração `ILogger` especificamente) — sem duplicar lógica de scrub. `FamiliaId`/`ContaId` são GUIDs opacos (não-PII); `Ip` já era logado em claro pelo `LogWarning` original (não é PII nova introduzida).
- **RequestId em exceção 500**: `GlobalExceptionHandler.LogErroInesperado` ganhou parâmetro `RequestId` (`httpContext.TraceIdentifier`) — argumento estruturado de `ILogger` vira `SentryEvent.Tags["RequestId"]` automaticamente (mesmo mecanismo já usado/scrubado, ver §1 acima). Correlação COMPLEMENTAR ao `X-Request-Id`/`BeginScope` de OBS-01 (§GAPS) — garante a tag no evento de erro mesmo se a propagação de scope pro Sentry não for automática pra todo call-site.
- Testes: `Tests/Infrastructure/Logging/{AlertaSegurancaSentryNuloTests,SentrySecurityAlertSinkTests}.cs`, `Tests/Api/Extensions/DependencyInjectionExtensionsSentryTests.cs` (resolução por DSN válido/ausente/Test), `Tests/Infrastructure/RefreshTokenServiceTests.cs` + `Tests/Api/Extensions/RateLimitRejeicaoLogTests.cs` (call sites).
...
|---|---|---|
| `statusCode >= 500` | `LogErroInesperado(logger, exception, message, requestId)` | `Error` (→ sink DB + Sentry, `RequestId` como tag desde 2026-08-09) |
...
### EF Core — log pelo pipeline da aplicação (T42, auditoria 2026-08-26)
- `AppDbContext` é construído manualmente (`InfrastructureExtensions.cs`, `AddScoped<AppDbContext>(sp => ...)`) — NÃO usa o helper `services.AddDbContext<T>(...)`, que normalmente encanaria `ILoggerFactory` sozinho via `UseApplicationServiceProvider`. Sem `.UseLoggerFactory(...)` explícito, o EF Core caía no `NullLoggerFactory` interno — os logs de comando/conexão do provider Npgsql nunca chegavam ao pipeline da app (nem ao sink de DB nem ao Sentry, ambos acima).
- Fix: `DbContextOptionsBuilder.UseLoggerFactory(sp.GetRequiredService<ILoggerFactory>())` (API confirmada via context7, `dotnet/entityframework.docs`) — mesmo `ILoggerFactory` do resto da app, então a categoria `Microsoft.EntityFrameworkCore.*` some pela mesma pipeline de sinks E é a mesma que o **anti-recursão** do sink de DB já filtrava por prefixo (§1 acima, `ErrorLogDbSinkProvider`) — antes do fix o filtro nunca disparava porque o log nem chegava lá.
- Teste de startup: `Tests/Infrastructure/DependencyInjection/EfCoreLoggingTests.cs` resolve `AppDbContext` do container montado por `AddInfrastructure` e assevera `context.GetService<IDbContextOptions>().FindExtension<CoreOptionsExtension>().LoggerFactory` = a MESMA instância registrada no DI (nunca uma `LoggerFactory` órfã criada ad-hoc).

### Gateway de agentes — diagnóstico preservado sem vazar pro cliente (T35, auditoria 2026-08-26)
`AgentExceptionFilter` (filtro externo do grupo `/internal/agents/v1`, [specification-agents] §5) captura QUALQUER exceção não tratada e responde `503 dependency_unavailable` com `detail` fixo — nunca `ex.Message` da exceção original (info-leak guard, mesma família do §Webhooks). Antes do fix, a substituição da exceção (`ex = new Exception(...)`) acontecia ANTES do `LogError`, então o log server-side também perdia o tipo/stack trace originais — diagnosticar a causa real exigia reproduzir localmente. Fix: captura `tipoOriginal`/`stackTraceOriginal` da exceção ANTES de substituí-la, loga os dois como parâmetros estruturados (`{TipoExcecao}`/`{StackTrace}`) — o `LogError` server-side agora carrega o diagnóstico completo, e o `detail` do `AgentProblem` devolvido ao gateway continua sem NENHUM dado da exceção original.
...
Sem PII crua em nenhum campo; conta-não-encontrada NÃO loga identificador. **Destino: stdout do container** — `LogWarning` fica abaixo do teto `>=Error` dos dois sinks (DB e Sentry, §1). **Exceção**: a linha "reuso detectado" (`RenovarSessaoHandler`/`RefreshTokenService`) tem captura explícita adicional no Sentry desde 2026-08-09 (`security_signal=refresh_reuse`, ver "Sinal de segurança explícito" acima) — as demais linhas da tabela seguem só no stdout.
...
### Rejeição do grupo de agentes (`/internal/agents/v1`)
`HmacSignatureFilter` registra **um** `LogWarning` por rejeição, categoria `forzion.tech.Api.Endpoints.Agents.Hmac.HmacSignatureFilter`, com `Motivo` (o `code` do contrato: `signature_invalid` \| `timestamp_out_of_window` \| `validation_failed`), `Metodo` e `Caminho`. **Nunca** a assinatura, o segredo ou o corpo — em nenhum nível (teste captura o sink a partir de `Trace` e reprova se algum deles aparecer).

Sem este log a rejeição é indiagnosticável dos dois lados: o chamador recebe `detail` fixo por `code` (info-leak guard, [specification-security] §7) e a divergência de payload canônico — o maior risco do handover — vira `401` mudo. Cross-ref [specification-agents] §3/§7.

**Destino: stdout do container**, mesma categoria da tabela auth-failure acima — `LogWarning` fica abaixo do teto `>=Error` dos dois sinks (DB e Sentry, §1) e NÃO tem captura explícita no Sentry. Deliberado por ora: rejeição de assinatura é ruído esperado durante integração/rotação, e alertar cada `401` de um endpoint fechado gera fadiga sem sinal. **Residual**: se o gateway começar a falhar em produção, o diagnóstico exige `docker logs` — promover a captura explícita (como `security_signal=refresh_reuse`) é aditivo e não muda o código do filtro.
...
- Mapeamento `Api/Extensions/RouteBuilderExtensions.cs` `MapHealthCheck` (ambos `AllowAnonymous().RequireRateLimiting("read")`; ordem no pipeline: depois de auth/authz/rateLimiter — ver [specification-backend] §UseApiConfiguration):
  ...
  - **Tag `agents-ready`** (desde 2026-08-19): ADITIVA sobre `db` e `schema` (os dois carregam `ready` E `agents-ready`; `stripe`/`resend`/`whatsapp` seguem só com `ready`). Consumida por `GET /internal/agents/v1/health`, que agrega SÓ essa tag: integração externa fora do ar não faz o gateway de agentes abrir circuito sem relação causal. Mapeamento próprio, distinto do readiness: `Healthy`→`200 healthy`, `Degraded`→`200 degraded`, `Unhealthy`→**`503` + `Problem{code=dependency_unavailable}`**; resposta sem nome de check, descrição ou exceção. Detalhe: [specification-agents] §6.
...
- **Alerting reativo por log — parcialmente resolvido (T4/#375, 2026-08-08 + `secprod-observabilidade`, 2026-08-09)**: chargeback (`LogCritical`), qualquer 500 (`GlobalExceptionHandler`, com `RequestId`) e os 2 sinais de segurança mais críticos (reuse de refresh, rejeição rate-limit `auth`/`mfa` — captura explícita, `security_signal` tag) chegam no Sentry backend, além do `error_logs`. **Ainda GAP**: as demais linhas de auth-failure (R1-R4 — senha inválida, e-mail não verificado, MFA/step-up falho) continuam `LogWarning` abaixo do teto `>=Error`, seguem só no stdout (deliberado — captura explícita pra CADA `LogWarning` do app inundaria o Sentry sem seletividade; só os 2 sinais mais acionáveis ganharam o tratamento). `BACKEND_SENTRY_DSN` ainda não setado no `.env` da VM (infra já lê a var — ação do operador). Sem regra de alerta/rota versionada no repo (config de dashboard Sentry). Sem alerta para DB down (readiness já reporta via `/health/ready`, §2, mas ninguém está inscrito num ping ativo) nem fila de e-mail atrasada.
