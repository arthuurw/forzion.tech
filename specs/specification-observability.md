...
- `ProcessarAsync` (ordering canônico, `specification-coding §1`): scope próprio → `config` via `IHealthReportConfigRepository.ObterAsync`; se null/!DeveEnviar → return. Senão: `Coletar` → `HealthSnapshot.Criar(...)` → `IHealthSnapshotRepository.AdicionarAsync` → `config.MarcarEnviado(agora)` → `IUnitOfWork.CommitAsync` → (try) `Sender.EnviarAsync(destinatarios)` + `LogInformation` (catch `OperationCanceledException` → propaga; catch demais → `LogCritical`, não reverte estado). Efeito externo SEMPRE depois do commit — falha de commit pós-envio duplicava e-mail no ciclo seguinte (fix histórico, card #314/HARD-01).
- `ExecutarRelatorioSaudeHandler` (`Application/UseCases/Admin/HealthReport/`, endpoint admin `POST /admin/health-report/run`, execução sob demanda): mesmo ordering — `Coletar` → `HealthSnapshot.Criar` → `AdicionarAsync` → `CommitAsync` → (try) `Sender.EnviarAsync` (catch `OperationCanceledException` → propaga; catch demais → `LogCritical`, retorna `Result.Success` mesmo assim). Card #320 (mesmo bug do #314, achado fora do escopo original).
- Testes: `Tests/Api/Services/RelatorioSaudeDiarioServiceTests.cs` (cobre `DeveEnviar` + ordering); `Tests/Application/Admin/HealthReport/ExecutarRelatorioSaudeHandlerTests.cs` (ordering + catch).
