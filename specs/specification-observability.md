...
- Registro `Api/Extensions/DependencyInjectionExtensions.cs`: `services.AddHealthChecks().AddDbContextCheck<AppDbContext>("db", tags: ["ready"])` (pacote `Microsoft.Extensions.Diagnostics.HealthChecks.EntityFrameworkCore` 8.0.11). Check `db` = `CanConnectAsync` do `AppDbContext`, só executado quando o endpoint readiness é chamado. Em ambiente `Test` o `AddInfrastructure` é pulado → `AppDbContext` não registrado → o check `db` só roda se o teste o registrar (ver `Tests/Api/Endpoints/HealthEndpointsTests.cs`).
- Mapeamento `Api/Extensions/RouteBuilderExtensions.cs` `MapHealthCheck` (ambos `AllowAnonymous().RequireRateLimiting("read")`; ordem no pipeline: depois de auth/authz/rateLimiter — ver [specification-backend] §UseApiConfiguration):
  ...
  - READINESS `/health/ready`: `HealthCheckOptions { Predicate = r => r.Tags.Contains("ready") }` → executa só o check `db`. `200 Healthy` quando o DB conecta; `503 Unhealthy` caso contrário.
