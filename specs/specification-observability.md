...
- **Persistência em LOTE** (PERF-07): o worker NÃO faz 1 `SaveChanges` por entry. `ProcessarCanalAsync` usa `WaitToReadAsync` + `TryRead` em loop p/ drenar até `TamanhoLote=100` itens já disponíveis no canal numa janela → `PersistirLoteAsync` adiciona todos no MESMO `AppDbContext` e faz 1 `SaveChangesAsync` (round-trip único por lote, não por log). Sob burst de erros (onde o sink mais importa) corta drasticamente o nº de round-trips. `agora`/`CreatedAt` (hora de inserção) é lido 1× por lote. Contagem de linhas persistidas é idêntica ao modo 1-a-1 (lote só agrupa o I/O).
  - **BLAST RADIUS ACEITO** (PERF-07, E6-i1, decisão do usuário 2026-06-18): o `catch {}` do `PersistirLoteAsync` engole a falha do `SaveChanges` do lote inteiro — uma falha transitória de DB descarta ATÉ `TamanhoLote=100` entries de uma vez (no modo 1-a-1 perdia 1). Esses descartes NÃO incrementam `DropsContados` (esse contador é só do overflow do canal, não de falha de persistência) → perda silenciosa do lote. ACEITO porque: `error_logs` é observabilidade best-effort, não fonte-de-verdade (sem dinheiro/estado); falha transitória de DB é rara e o próprio erro que motivou o log normalmente reaparece; propagar/logar dentro do sink causaria recursão (a razão do `catch {}`). 3 eixos: segurança = nenhuma (dados de erro, não PII crítica de negócio); performance = lote é o ganho headline sob burst; usabilidade(ops) = janela de cegueira no pior caso (≤100 erros perdidos num blip de DB) tolerada vs complexidade de fallback. Hardening futuro se incomodar: fallback per-entry ou retry curto antes de descartar o lote.
...
- `MarcarErro` (rejeição do gov 4xx) → `Warning` (nota visível em `Erro`, reprocessável; pagamento intacto). Timeout/5xx → exceção propaga p/ retry do outbox (sem log de erro terminal).
