...
- **Call sites**: `RefreshTokenService.Reuse` → `Registrar("refresh_reuse", ..., {FamiliaId, ContaId})`; `RegistrarRejeicaoAuth` (`DependencyInjectionExtensions.cs`, `OnRejected` do RateLimiter) → `Registrar("rate_limit_rejection", ..., {Politica, Rota, Ip})`, só pras políticas `auth`/`mfa`.
