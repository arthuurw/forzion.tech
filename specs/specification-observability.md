...
- Backend: logging estruturado via `ILogger` + sink ERROR→DB; `/health` liveness + `/health/ready` readiness (DbContextCheck + SchemaHealthCheck + StripeHealthCheck + ResendHealthCheck + WhatsAppHealthCheck); relatório de saúde diário por e-mail; alert de chargeback via LogCritical; auth-failure LogWarning (R1-R4); auditoria durável de ações privilegiadas (R10-R14, `logs_aprovacao`). **[GAP] Backend NÃO tem Sentry nem qualquer agregador externo de log/erro**: zero referência em `forzion.tech.Api/*.csproj`, `Program.cs`, `forzion.tech.Infrastructure/*.csproj`; `docker-compose.server.yml` não injeta DSN algum. Consequência: TODO `LogWarning`/`LogCritical` do backend (chargeback, auth-failure R1-R4, reuse-detection de refresh token) fica só no stdout do container + no sink `error_logs` (que persiste só Error/Critical — §1) — nenhum alerta real dispara hoje em produção nem homolog.
...
Sem PII crua em nenhum campo; conta-não-encontrada NÃO loga identificador. **Destino hoje: stdout do container apenas** (ver ESTADO GERAL — sem Sentry no backend).
...
- **Alerting reativo só por log — sem destino real**: backend não tem Sentry (ESTADO GERAL) ⇒ chargeback (`LogCritical`), auth-failure (R1-R4) e reuse-detection de refresh token ficam só no stdout do container + sink `error_logs`; nenhum agregador externo recebe. Sem regra de alerta versionada no repo. Sem alertas para 500s, DB down, fila de e-mail.
