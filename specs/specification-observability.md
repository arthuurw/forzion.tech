...
- **Canal bounded + worker**: `Log` enfileira `LogEntry` em `Channel<LogEntry>` (capacidade 1000, `DropWrite`); worker background (`Task.Run`) drena e persiste sequencialmente. Overflow descarta o item novo e incrementa `DropsContados` (nunca silencioso). `catch {}` no `PersistirAsync` engole tudo — NUNCA propaga nem loga (evita recursão).
- **Drain no shutdown**: `IHostApplicationLifetime.ApplicationStopping.Register(DrenaNoShutdown)` — ao SIGTERM: `Writer.TryComplete()` + `Wait(5 s)` para esgotar os itens restantes antes de o processo sair. Timeout de 5 s evita bloquear shutdown com DB indisponível.
