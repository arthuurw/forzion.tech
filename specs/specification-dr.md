...
- [ALVO/em execução — script pronto 2026-07-08] Drill do artefato REAL do R2 (`scripts/dr/restore-drill.ps1`, §2 RUNBOOK PRIMÁRIO) — script + runbook commitados; EXECUÇÃO pendente (requer identity age offline + creds R2 do operador, fora do alcance de agente/CI por design). Ao rodar: validar `__EFMigrationsHistory` = migrations aplicadas no dump — total canônico em [specification-db §STACK]; 31 no drill 2026-06-11 — + contagem `contas`, e registrar em §DRILL LOG.
