...
- `CriarPagamentoComIntentService` (Application/Services) — **orquestrador CANÔNICO da cobrança** (G-PAY-1, PERF-01); todos os callers (aluno + treinador) passam por ele via `CriarPagamentoComIntentParams<TPagamento>` (delegates: `CriarIntent`, `ObterPendente`, `VerificarIdempotencia`, `CriarPagamento`, `AplicarIntent`, `AdicionarAsync`, `MarcarFalhou`). Ordem: (1) **pré-check** sem tx — `ObterPendente`; se reusável (`VerificarIdempotencia` ≠ null, i.e. intent já setado) retorna SEM tocar Stripe; (2) `CriarIntent` (chamada Stripe) **FORA** da tx; (3) loop de retry (≤3) sobre uma tx `Serializable` CURTA: `ObterPendente` autoritativo → reuse | `MarcarFalhou`(zumbi) → `CriarPagamento` → `AplicarIntent`(dados do intent) → `AdicionarAsync` → `CommitAsync` → `tx.Commit`. Conflito (`IDatabaseErrorInspector.EhViolacaoDeUnicidade` 23505 OU `EhConflitoDeSerializacao` 40001) → backoff `50ms*tentativa` (`TimeProvider`) + retry; intent NÃO é recriado por tentativa (criado 1× antes do loop). PERF-01: nenhuma chamada Stripe ocorre com tx aberta (HTTP lento não segura lock Serializable). Idem-key vem do caller (§IDEMPOTÊNCIA DA COBRANÇA). `IDatabaseErrorInspector` (Application/Interfaces, impl Infra) mantém Npgsql/EF fora da Application.
- `GerarCobrancaMensalHandler` (Application/UseCases/Pagamentos/GerarCobrancaMensal) — monta `CriarPagamentoComIntentParams` e delega a `CriarPagamentoComIntentService`. `CriarIntent` ramifica por `command.Metodo` → `CriarPix/CartaoPaymentIntentAsync(assinatura.Valor, accountId, taxa, chave, ct)` capturando o resultado; `AplicarIntent` aplica `DefinirDadosPix/Cartao`. Idem-key `cobr:aluno:{assinatura.Id}:{minuto}`.
...
- `IniciarPagamentoPlanoHandler` (Application/UseCases/Treinadores/IniciarPagamentoPlano) — fluxo CADASTRO: gate `Treinador.Status==AguardandoPagamento` + AssinaturaTreinador Pendente. Delega a `CriarPagamentoComIntentService` (G-PAY-1/PERF-01): `CriarPagamento` = `PagamentoTreinador.Criar(Cadastro)`; `CriarIntent` = `CriarPix/CartaoPlataformaPaymentIntentAsync(assinatura.Valor, chave, ct)`. Idem-key `cobr:treinador-cadastro:{assinatura.Id}:{minuto}`. Valor vem da assinatura (nunca do caller).
- `ContratarPlanoTreinadorHandler` (Application/UseCases/Treinadores/ContratarPlanoTreinador) — fluxo CONTRATAÇÃO: treinador `Ativo` sem assinatura ativa contrata plano. Cria `AssinaturaTreinador(Pendente)` + `PagamentoTreinador(Contratacao)` via `CriarPagamentoComIntentService`. Idempotência: `ObterAtual` Pendente+mesmo plano → reusa sem novo intent; 23505 → re-fetch vencedor (AD-002, [specification-db]). `TreinadorId=userContext.PerfilId` (nunca do body). Valor = `plano.Preco` (server-derived).
- `GerarCobrancaPlanoTreinadorHandler` (Application/UseCases/Treinadores/GerarCobrancaPlanoTreinador) — RENOVAÇÃO mensal: chamado pelo cron `/internal/processar-renovacoes-treinador` por assinatura a vencer. Gate: rejeita Cancelada/Pendente. **Aplica downgrade agendado ANTES de cobrar**: se `PlanoPlataformaIdAgendado` setado → `AplicarPlanoAgendado(planoAgendado.Preco)` (promove agendado→atual + valor); plano agendado com `Preco≤0` (Free) → `assinatura.Cancelar` SEM cobrança (encerra); plano agendado excluído da plataforma → `LimparPlanoAgendado` + prossegue com plano atual. Depois delega a `CriarPagamentoComIntentService`: `PagamentoTreinador(Renovacao, valor=assinatura.Valor)` + `CriarPix/CartaoPlataformaPaymentIntentAsync(assinatura.Valor, chave, ct)`. Idem-key `cobr:treinador:{assinatura.Id}:{minuto}`.
...
- API client (`src/lib/api/pagamento.ts`): `iniciarOnboarding`, `verificarOnboarding`, `gerarCobranca`, `obterPagamento`, `listarPagamentosAssinatura`, `obterMinhaAssinatura`, `contratarPlano`.
