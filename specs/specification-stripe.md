...
- `IStripeService` (Application/Interfaces): 11 métodos:
...
- **Treinador self-service (R2)**: `CancelarMinhaAssinaturaTreinadorHandler` (Application/UseCases/Treinadores/CancelarMinhaAssinaturaTreinador) injeta `IAssinaturaTreinadorRepository` + `IVinculoTreinadorAlunoRepository` + `IPagamentoTreinadorRepository` + `IStripeService` + uow/timeProvider/logger. Endpoint `POST /treinador/plano/cancelar` (JWT treinador, sem body). Fluxo: (1) `ObterAtualPorTreinadorAsync` → null/Cancelada ⇒ `Error(NotFound)` (404 `assinatura_nao_encontrada`); (2) gate D2 `TemVinculosAtivosAsync(treinadorId)` true ⇒ `AssinaturaTreinadorErrors.OffboardingNecessario` (422 `offboarding_necessario`, sem cascata de cancelamento dos alunos); (3) `assinatura.Cancelar(agora)` → `CommitAsync`; (4) **pós-commit** (espelha CR#1 do aluno), se `(agora - DataInicio).TotalDays ≤ 7` → `ObterPagoPorAssinaturaAsync` (1º `Pago` com `StripePaymentIntentId`, ordem `CreatedAt`) → `CriarReembolsoAsync(intentId, reverterTransferencia: false, ct)` — charge direto-plataforma, **sem** reverse transfer (G1). Falha Stripe = `LogCritical` + prossegue. Resposta 200 = `{ canceladaEm: <DateTime> }`.
