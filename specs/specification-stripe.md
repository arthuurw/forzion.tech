...
- `IStripeService` (Application/Interfaces): 12 métodos:
  ...
  - `CriarPixPaymentIntentAsync(valor, accountId, taxaPlataformaPercent, idempotencyKey, ct)` → `PixPaymentResult(intentId, qrCode, qrCodeUrl, expiracao)` (fluxo aluno — Connect + fee split). `idempotencyKey` explícita do caller (não derivada de `pagamentoId`) — ver §IDEMPOTÊNCIA DA COBRANÇA.
  - `CriarCartaoPaymentIntentAsync(valor, accountId, taxaPlataformaPercent, idempotencyKey, ct)` → `CartaoPaymentResult(intentId, clientSecret)` (fluxo aluno — Connect + fee split). `idempotencyKey` explícita do caller.
...
- `Stripe-Idempotency-Key` header passado em todos os `Criar*PaymentIntentAsync` via `PaymentIntentRequestOptions(idempotencyKey)` — key DETERMINÍSTICA fornecida pelo caller (não mais derivada de `pagamentoId`/`pagamentoTreinadorId`). Ver §IDEMPOTÊNCIA DA COBRANÇA pro esquema. Belt-and-suspenders: retry de network/transport NÃO cria 2º PaymentIntent (Stripe responde idêntico até 24h depois com mesma key). `CriarContaConnectAsync`/`GerarLinkOnboardingAsync` ficam sem key (idempotency menos crítico). Refund mantém `refund-{guid_n}` (`RefundRequestOptions(pagamentoId)`).

## IDEMPOTÊNCIA DA COBRANÇA (idem-key determinística — PERF-01)
A idem-key do PaymentIntent é DETERMINÍSTICA e montada pelo CALLER, não derivada do `Pagamento.Id` (que muda a cada `Criar`, anulando a dedup do Stripe sob retry/double-tap). Esquema: `cobr:{tipo}:{assinaturaId}[:{discriminador}]:{bucket}`, com `bucket = now.ToString("yyyyMMddHHmm")` (UTC, minuto da tentativa).
- **Discriminador escolhido = BUCKET DE MINUTO** (supersede o antigo `DataProximaCobranca`). Semântica: double-tap/concorrência no MESMO minuto ⇒ Stripe dedup (1 intent); re-tentativa após expiração do Pix (>1h) ⇒ minuto novo ⇒ intent novo (correto, pendente velho já é zumbi); ciclos consecutivos ⇒ buckets distintos (sem colisão). Estável sob o retry de transporte do Stripe (janela de 24h) dentro do minuto.
- Chaves por caller:
  - aluno (renovação/manual): `cobr:aluno:{assinatura.Id}:{minuto}` — `GerarCobrancaMensalHandler`.
  - treinador (renovação): `cobr:treinador:{assinatura.Id}:{minuto}` — `GerarCobrancaPlanoTreinadorHandler`.
...
- `ProcessarWebhookStripeHandler` (Application/UseCases/Pagamentos/ProcessarWebhookStripe) — despacha os eventos do webhook. Os 3 `payment_intent.*` BIFURCAM por `evento.TipoMetadata == "plano_treinador"` (metadata `tipo`): plano do treinador → `ProcessarPagamentoTreinador{Pago,Falhou,Transicao}Async` (sem cross-account); aluno → caminho Connect. **Aluno**: succeeded → MarcarPago + RegistrarPagamentoRegularizado + (se Pendente) Ativar + AgendarProximaCobranca(+1mês); payment_failed → MarcarFalhou + RegistrarPagamentoFalho; canceled → MarcarExpirado (auto após Pix `ExpiresAfterSeconds=3600`). **Plano treinador**: succeeded ramifica por `Finalidade` — `Cadastro`→`FinalizarCadastroAsync` (Ativar + Agendar + `ConfirmarPagamentoPlano` + `Conta.EmitirRegistro` que dispara a verificação de e-mail adiada), `Contratacao`→`FinalizarContratacaoAsync` (Ativar + Agendar; NÃO chama `ConfirmarPagamentoPlano`/`EmitirRegistro`, TreinadorStatus inalterado); **ambas NO MESMO commit**; failed/canceled espelham o aluno. Comuns: `account.updated` chargesEnabled=true → ConfirmarOnboarding; chargesEnabled=false → só `LogCritical` se `OnboardingCompleto==true` (conta estava live e caiu), senão silencioso (false é normal no onboarding) — ambos `JaConsistente`; `charge.refunded` → MarcarEstornado; `charge.dispute.created` → MarcarEmDisputa + Ativa→Inadimplente + evidência automática. Retorna `ProcessarEventoResultado` (Aplicado/JaConsistente/Ignorado). Idempotência/cross-account/fail-loud em §FLUXOS Webhook. Enumerar branches em `ProcessarWebhookStripeHandler.cs`.
...
- `CriarPagamentoComIntentService` (Application/Services) — **orquestrador CANÔNICO da cobrança** (G-PAY-1, PERF-01); todos os callers (aluno + treinador) passam por ele via `CriarPagamentoComIntentParams<TPagamento>` (delegates de criar/obter/verificar/aplicar). Ordem CRÍTICA: pré-check de pendente reusável SEM tx → `CriarIntent` (Stripe) **FORA** da tx → tx `Serializable` curta com retry (≤3) sobre conflito (`IDatabaseErrorInspector` 23505/40001, backoff `50ms*tentativa`). **PERF-01: nenhuma chamada Stripe com tx aberta** (HTTP lento não segura lock Serializable); intent criado 1× antes do loop (não recriado por tentativa). Idem-key vem do caller (§IDEMPOTÊNCIA DA COBRANÇA). `IDatabaseErrorInspector` (impl Infra) mantém Npgsql/EF fora da Application. Detalhe em [specification-concurrency §4].
- `GerarCobrancaMensalHandler` (Application/UseCases/Pagamentos/GerarCobrancaMensal) — monta `CriarPagamentoComIntentParams` e delega a `CriarPagamentoComIntentService`; `CriarIntent` ramifica por `command.Metodo` (Pix/Cartão, `assinatura.Valor` + Connect). Idem-key `cobr:aluno:{assinatura.Id}:{minuto}`.
...
- `PagamentoTreinador` (Domain/Entities) — agregado: Status (Pendente → Pago/Falhou/Expirado; Pago → Estornado/EmDisputa), `Finalidade` (enum `FinalidadePagamentoTreinador`: `Cadastro=0`/`Renovacao=1`/`TrocaPlano=2`/`Contratacao=3`), `Metodo`, `StripePaymentIntentId`. `Criar(treinadorId, assinaturaTreinadorId, valor, finalidade, agora, metodo)`; `DefinirDadosPix`/`DefinirDadosCartao`; `MarcarPago`/`MarcarFalhou`/`MarcarExpirado`. `MarcarPago` dispara `PagamentoTreinadorPagoEvent`. **T4**: `MarcarEstornado(agora)` — só de Pago; o handler `ProcessarEstornoTreinadorAsync` adicionalmente chama `assinatura.MarcarInadimplentePorDisputa` para congelar acesso do treinador. `MarcarEmDisputa(agora)` — só de Pago; handler `ProcessarDisputaTreinadorAsync` idem. **NFS-e T16/T17**: ambas as transições disparam `PagamentoTreinadorEstornadoEvent`/`PagamentoTreinadorEmDisputaEvent(PagamentoTreinadorId, TreinadorId, Valor, OcorridoEm)` — consumidos pelo durável `CancelarNfseHandler` (Application/UseCases/Nfse/CancelarNfse) que, havendo NFS-e `Emitida` da comissão, faz `SolicitarCancelamento` + enfileira `fx:cancelar_nfse`. Arthur segue monitorando estorno/disputa do plano via log (sem e-mail de notificação).
...
- `IniciarPagamentoPlanoHandler` (Application/UseCases/Treinadores/IniciarPagamentoPlano) — fluxo CADASTRO: gate `Treinador.Status==AguardandoPagamento` + AssinaturaTreinador Pendente. Delega a `CriarPagamentoComIntentService` (`CriarPagamento`=`PagamentoTreinador.Criar(Cadastro)`, `CriarIntent`=`Criar*PlataformaPaymentIntentAsync`). Idem-key `cobr:treinador-cadastro:{assinatura.Id}:{minuto}`. Valor da assinatura (nunca do caller).
- `ContratarPlanoTreinadorHandler` (Application/UseCases/Treinadores/ContratarPlanoTreinador) — fluxo CONTRATAÇÃO: treinador `Ativo` sem assinatura ativa contrata plano; cria `AssinaturaTreinador(Pendente)` + `PagamentoTreinador(Contratacao)` via `CriarPagamentoComIntentService`. Idempotência: `ObterAtual` Pendente+mesmo plano → reusa sem novo intent; 23505 → re-fetch vencedor (AD-002). `TreinadorId=userContext.PerfilId` e valor=`plano.Preco` (server-derived, nunca do body).
- `GerarCobrancaPlanoTreinadorHandler` (Application/UseCases/Treinadores/GerarCobrancaPlanoTreinador) — RENOVAÇÃO mensal (cron `/internal/processar-renovacoes-treinador`). Gate: rejeita Cancelada/Pendente. **Aplica downgrade agendado ANTES de cobrar**: `PlanoPlataformaIdAgendado` setado → `AplicarPlanoAgendado` (promove agendado→atual+valor); agendado Free (`Preco≤0`) → `Cancelar` SEM cobrança; agendado excluído da plataforma → `LimparPlanoAgendado` + segue com atual. Depois delega a `CriarPagamentoComIntentService` (`PagamentoTreinador(Renovacao, valor=assinatura.Valor)`). Idem-key `cobr:treinador:{assinatura.Id}:{minuto}`.
- `TrocarPlanoTreinadorHandler` (Application/UseCases/Treinadores/TrocarPlanoTreinador) — TROCA de plano. **Valor SEMPRE server-derived** (`PlanoPlataforma.Preco`/proração; nunca do caller). Gates: plano existe+ativo+≠Elite; assinatura ≠Cancelada; ≠mesmo plano. Ramifica 3-vias:
  - **Inadimplente** (`ProcessarInadimplenteAsync`): regulariza trocando — `PagamentoTreinador(TrocaPlano, valor=novoPlano.Preco CHEIO, PlanoAlvoId)` + PaymentIntent plataforma via `CriarPagamentoComIntentService` (G-PAY-1/PERF-01). Idem-key `cobr:troca:{assinatura.Id}:{novoPlano.Id}:{minuto}`. Webhook pago → handler aplica `RegistrarPagamentoRegularizado` + `TrocarPlanoImediato` + `AgendarProximaCobranca(+1mês)`.
  - **Upgrade** (`novoPlano.Preco > assinatura.Valor`, `ProcessarUpgradeAsync`): proração COBRADA AGORA = `Round((novoPlano.Preco − Valor) × diasRestantes / 30, 2, AwayFromZero)` (`diasRestantes = max(0, DataProximaCobranca − agora)`). Proração ≤0 → `TrocarPlanoImediato` sem cobrança (commit direto). Proração >0 → `PagamentoTreinador(TrocaPlano, valor=proração, PlanoAlvoId)` + PaymentIntent via `CriarPagamentoComIntentService` (re-uso idempotente se pendente mesma TrocaPlano+PlanoAlvo+intent≠null; senão Falhou+novo). Idem-key `cobr:troca:{assinatura.Id}:{novoPlano.Id}:{minuto}`.
...
3. Handler → `CriarPagamentoComIntentService`: pré-check de pendente reusável → Stripe (CriarPix/CartaoPaymentIntent, com ApplicationFeeAmount+TransferData) FORA da tx → tx Serializable curta (reuse|MarcarFalhou+Criar+aplica intent+commit) com retry de 23505/40001. Ver §COMPONENTES.
...
- **Idempotência sob redelivery CONCORRENTE (xmin) (IDEMP-04)**: o curto-circuito por estado terminal é check-then-act — duas entregas do MESMO evento em PARALELO leem `Pagamento`/`PagamentoTreinador` ainda `Pendente` (guard passa nas duas) e ambas tentam commitar a transição. `Pagamento` e `PagamentoTreinador` mapeiam `xmin` (concurrency token, migration `AdicionarConcurrencyTokenPagamentos`): a 2ª perde a corrida → `CommitAsync` lança `DbUpdateConcurrencyException`. `CommitarTransicaoPagamentoAsync` envolve cada commit de transição de pagamento (11 pontos); `catch when EhConflitoDeConcorrenciaOtimista(ex)` → `LogDebug` + `JaConsistente` (efeito da vencedora já aplicado → idempotente, sem 500). `account.updated` NÃO é coberto (sem token xmin). Prova: `ConcurrentBillingRaceTests` (E2E paralelo real). [canônico em concurrency §2/§3].
...
| GET /treinador/modo-pagamento/preview | Treinador | ObterPreviewModoPagamentoTreinadorHandler | 200 `{assinaturasAtivasAlunos, vinculosCobravelSemAssinatura}` (read-only; espelha a contagem que `AlterarModoPagamento` aplicaria; handler retorna a response direto — sem `Result`/404, treinador autenticado sempre existe) | 403 |
...
| GET /treinador/pagamentos/recebimentos?cursor&tamanho | Treinador | ListarRecebimentosTreinadorHandler | 200 `{itens[], proximoCursor, taxaPlataformaPercent}` — paginação keyset `(CreatedAt desc, Id)`, cursor **Base64Url** (`-_`, sem padding — robusto a `+`/`/` virando espaço em query string), escopo `userContext.PerfilId` (anti-IDOR); lista os 6 status; item traz bruto, status, aluno, método, datas; `taxaPercent`/`liquidoEstimado` **nullable** — `null` em `Falhou`/`Expirado` (nunca houve recebimento → frontend mostra "—"). Líquido segue a disciplina CANÔNICA de centavos (`MoneyCentavos.ValorETaxaCentavos`: `bruto_centavos − floor(floor(bruto×100)×taxa/100)`), idêntica à comissão mensal — NÃO `bruto×(1−taxa/100)` em reais (divergia 1 centavo) | 403 |
| POST /treinador/plano/contratar | Treinador | ContratarPlanoTreinadorHandler | 200 `{pagamentoId, valorPagamento, metodoPagamento, pixQrCode?, pixQrCodeUrl?, pixExpiracao?, clientSecret?}` | 404, 409 (`treinador.nao_elegivel_contratacao`, `treinador.assinatura_ja_existe`), 422 (`plano_plataforma.inativo`, `EliteIndisponivel`) |
...
## FRONTEND
- Componentes `src/components/pagamento/` (`PagamentoCartao.tsx` = Stripe.js `<Elements>`/`<PaymentElement>`/`confirmPayment`; `PagamentoPix.tsx` = QR + polling 30s), páginas `src/app/(treinador|aluno)/` (pagamentos/onboarding), API client `src/lib/api/pagamento.ts` — enumerar via `ls`. Dados de cartão só client-side (PCI SAQ-A, §SEGURANÇA).
- E2E (`frontend/e2e/specs/critical/`): `checkout-stripe.spec.ts` (F3 — seed via `garantirPagamentoPendente()`), `treinador-onboarding-stripe.spec.ts` (F29). Test cards em `frontend/e2e/utils/stripe.ts`.
...
- `RequireAssinaturaAtivaFilter : RequireAssinaturaAtivaFilterBase` — impl aluno (`CodigoErro = "ASSINATURA_INADIMPLENTE"`). `TipoConta != Aluno` → false. Resolve aluno via `IAlunoRepository.ObterPorContaIdAsync`, depois `IAssinaturaAlunoRepository.ObterAtualPorAlunoAsync` (single-row no DB: última não-Cancelada por `CreatedAt`). Inadimplente → 403. (Era `ListarPorAlunoAsync` + `MaxBy` em memória — caminho QUENTE; trocado na remediação 2026-06-24, ver [specification-performance] §1 "Filtro no banco".)
- `RequireAssinaturaTreinadorAtivaFilter : RequireAssinaturaAtivaFilterBase` — impl treinador (`CodigoErro = "ASSINATURA_TREINADOR_INADIMPLENTE"`). `TipoConta != Treinador` → false (aluno/admin bypassam). Resolve `IAssinaturaTreinadorRepository.ObterAtualPorTreinadorAsync(PerfilId)` (single-row no DB) → `Status == Inadimplente` → 403.
- Aplicado em (consumo de feature paga do treinador): aluno → `POST /aluno/execucoes`. Treinador → `POST /treinador/vinculos/{id}/aprovar`, `POST /treinador/alunos/{alunoId}/reativar`, `POST /treinador/alunos/{alunoId}/fichas/{treinoId}`, `POST /treinos`, `POST /treinos/{id}/vincular-aluno`, `POST /treinos/{id}/duplicar`. Princípio: bloquear **consumo/entrega de feature paga** (aceitar/servir aluno + criar/atribuir ficha), NUNCA leitura (GET liberado pela base — LGPD/CDC) nem regularização: `/treinador/plano*`, `/treinador/pagamentos*`, `/treinador/onboarding*`, `/treinador/dados-fiscais`, `/treinador/notas-fiscais*`, `modo-pagamento`, `pacotes` (setup comercial) e `desvincular` (remove acesso, não consome) ficam liberados.
...
- Integration (Docker): `ConcurrentBillingRaceTests` (F12 — prova IDEMP-04, 2 tasks paralelas + Testcontainers Postgres).
- Frontend/E2E: `PagamentoCartao.test.tsx`, `(aluno)/__tests__/pagamento.test.tsx` (MSW), `checkout-stripe.spec.ts` + `treinador-onboarding-stripe.spec.ts` (F3/F29).
- Snapshots (F33 — breaking change no DTO falha aqui): `ResponseDtoSnapshots.PagamentoResponse_Aluno_IncluiClientSecret` + `..._Treinador_OmiteClientSecret`.
...
- Currency `"brl"` e fator ×100 hardcoded em todos os `Criar*PaymentIntent` (MN-5). Correto para BR-only (`Country="BR"` no Connect Express) — sem multi-currency por design. Centralizar se o escopo mudar.
