...
- **Resposta automática a chargeback (R9)**: após marcar a disputa e commitar, o handler chama `EnviarEvidenciaDisputaAsync(disputeId, evidencias)` nos DOIS ramos (`ProcessarDisputaCriadaAsync` aluno → `EnviarEvidenciaDisputaAlunoAsync`; `ProcessarDisputaTreinadorAsync` → `EnviarEvidenciaDisputaTreinadorAsync`). Evidências coletadas sem nova coluna (D9): `EmailCliente` (Conta.Email — aluno via `IAlunoRepository → ContaId → Conta`; treinador via `Treinador.ContaId → Conta`), `DataAtivacao` (`assinatura.DataInicio` → Stripe `ServiceDate`), `DataUltimaAtividade`/`DataUltimoPagamento` (`pagamento.DataPagamento`). Stripe API = `Dispute.Update(disputeId, { Evidence })` — **disputeId, não chargeId**. Falha no envio = `LogCritical` + NÃO bloqueia (disputa já marcada/commitada). Sem `disputeId` no payload → não envia (silencioso).
