...
- `PagamentoTreinador` (Domain/Entities) — agregado: Status (Pendente → Pago/Falhou/Expirado; Pago → Estornado/EmDisputa), `Finalidade` (enum `FinalidadePagamentoTreinador`: `Cadastro=0`/`Renovacao=1`/`TrocaPlano=2`), `Metodo`, `StripePaymentIntentId`. `Criar(treinadorId, assinaturaTreinadorId, valor, finalidade, agora, metodo)`; `DefinirDadosPix`/`DefinirDadosCartao`; `MarcarPago`/`MarcarFalhou`/`MarcarExpirado`. `MarcarPago` dispara `PagamentoTreinadorPagoEvent`. **T4**: `MarcarEstornado(agora)` — só de Pago; o handler `ProcessarEstornoTreinadorAsync` adicionalmente chama `assinatura.MarcarInadimplentePorDisputa` para congelar acesso do treinador. `MarcarEmDisputa(agora)` — só de Pago; handler `ProcessarDisputaTreinadorAsync` idem. **NFS-e T16/T17**: ambas as transições disparam `PagamentoTreinadorEstornadoEvent`/`PagamentoTreinadorEmDisputaEvent(PagamentoTreinadorId, TreinadorId, Valor, OcorridoEm)` — consumidos pelo durável `CancelarNfseHandler` (Application/UseCases/Nfse/CancelarNfse) que, havendo NFS-e `Emitida` da comissão, faz `SolicitarCancelamento` + enfileira `fx:cancelar_nfse`. Arthur segue monitorando estorno/disputa do plano via log (sem e-mail de notificação).
