...
- `CancelarNfseEfeitoHandler` (worker): guard `PrazoCancelamentoDias` LOCAL (fast-path) ANTES de chamar o gov → sucesso `MarcarCancelada`; rejeição do gov (ex. E8001 prazo) OU prazo local estourado → `MarcarCancelamentoExpirado` + `LogCritical` (estado terminal não-cancelado, exige ajuste fiscal manual do contador). Endpoint gov: `POST /nfse/{chave}/eventos`, evento `101101`, XML assinado+gzip+b64 (mesmo pipeline da emissão).
  - **FAIL-LOUD simétrico ao emit (IDEMP-01/G1)**: `MarcarCancelada` vindo `IsFailure` por estado inesperado ⇒ `throw` (não `log+return`) → retry do outbox. Na prática a transição é inalcançável via entidade real (o guard `Status==CancelamentoSolicitado` do handler está alinhado ao da transição, sem condição secundária) — o throw é defesa simétrica ao emit, não um caminho exercitado por teste (o emit é o reachable, L81).
...
- Impl `ViaCepConsultaCepService` (Infrastructure/Services): normaliza CEP a 8 dígitos via `Domain.Shared.Digitos.Apenas` (curto-circuita lixo sem rede); `GET {ViaCep:UrlBase}{cep}/json/`. ViaCEP retorna 200 mesmo p/ CEP inexistente (corpo `{erro:true}`) → `BoolTolerante` (`JsonConverter<bool>`) aceita o `erro` como bool OU string (`"true"`) pois o campo varia no contrato ViaCEP. Sem retry (autofill é best-effort, não outbox). GOTCHA: `InvalidOperationException` (ex.: `BaseAddress` ausente quando `ViaCep:UrlBase` em branco → `GetAsync` relativo lança) está no catch → degrada para `ServicoIndisponivel` (502), nunca 500.
- Endpoint `GET /treinador/cep/{cep}` (group `Treinador`, `RequireRateLimiting("read")`): `IsFailure → ToProblemResult()` (status derivado do `ErrorType`: 400/404/**502**); sucesso → `ConsultaCepResponse`. SEM special-case inline — o 502 sai do mapa `ExternalService` ([specification-backend] §ErrorType). Auth do grupo já restringe a treinador logado.
...
- Frontend: `dados-fiscais/page.tsx` → `nfseApi.consultarCep` (`lib/api/nfse.ts`) dispara quando o CEP atinge 8 dígitos (`soDigitos`); `AbortController` cancela a busca anterior (digitação rápida não corre). Popula `logradouro/bairro/uf(upper)/codigoMunicipioIbge` (+`complemento`) SÓ quando o campo vier não-vazio — CEP único/geral (logradouro/bairro `""`) NÃO apaga valor já digitado. `localidade` NÃO tem campo no form (município carregado pelo IBGE). Falha: 404 → "CEP não encontrado, confira o número digitado."; demais (502/rede) → "Não foi possível buscar o CEP, preencha o endereço manualmente." (distinção via `extractApiErrorInfo(err).status`; não bloqueia entrada manual).
...
- DPS XML CONFERIDO no XSD 1.01 (`manual/.../Schemas/1.01`): raiz `<DPS versao="1.01">`, `infDPS@Id` = `DPS`+cMun(7)+tipoInscFed(1)+InscFed(14)+serie(5)+nDPS(15) = 45 chars. Assinatura `ds:Signature` enveloped + C14N, RSA-SHA1 (padrão SPED — não trocar p/ SHA256 sem o gov aceitar). `EmissorNfseNacionalService.Assinar` roda em AMBOS os caminhos (`EmitirAsync`→`MontarAssinarCompactar` e `CancelarAsync`→`MontarAssinarCompactarEvento`). RESOURCE-HYGIENE (remediação 2026-06-24): `certificado.GetRSAPrivateKey()` aloca uma instância RSA descartável NOVA por chamada — sob `EphemeralKeySet` (runtime Linux/prod) cada `Assinar` cria handle nativo + heap; é disposta via `using var rsa` após `GetXml()` (sem dispose, acumula por nota emitida/cancelada). NÃO dispor o `X509Certificate2` (`_certificado.Value`) aqui — é o `Lazy` do serviço, disposto em `Dispose(bool)`. `SignedXml` NÃO é `IDisposable` no .NET.
