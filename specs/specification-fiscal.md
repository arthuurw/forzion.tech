...
- Máquina de estado (`enum NotaFiscalStatus {Pendente=0, Emitida=1, Erro=2, BloqueadaDadosFiscais=3, CancelamentoSolicitado=4, Cancelada=5, CancelamentoExpirado=6}`), transições guard'd (`Result` + `NotaFiscalErrors`):
  ...
  - `SolicitarCancelamento(agora)`: Emitida→CancelamentoSolicitado.
...
- `INotaFiscalRepository`: `AdicionarAsync`, `ObterPorIdAsync`, `ObterPorPagamentoTreinadorAsync`, `ListarPorTreinadorAsync`(keyset), `ListarPorStatusAsync`(keyset), `ListarAdminAsync`(filtro+keyset), `ExisteComissaoAsync(treinador, competencia)`.
...
- **Soma por pagamento truncada** `floor(valor*taxa)` em centavos (`Math.Floor`/SQL `FLOOR`, `MoneyCentavos`) — espelha o que o Stripe cobrou em CADA PaymentIntent; truncar o agregado divergiria ~1 centavo/pagamento (inaceitável p/ doc fiscal).
...
- `CancelarNfseHandler` (Application, UMA classe p/ os DOIS eventos; roteamento explícito `IDomainEventHandlerBase.HandleAsync` p/ evitar CS8705): localiza `NotaFiscal Emitida` por `pagamentoTreinadorId`, `SolicitarCancelamento` + enqueue `fx:cancelar_nfse:{notaId}`. Durável.
...
- Reemissão é segura: `NumeroDpsEstavel()` gera o MESMO DPS → gov dedup; + UNIQUE + chave outbox + estado terminal garantem 1 nota.
