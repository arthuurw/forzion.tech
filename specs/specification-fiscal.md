...
- GOTCHA: efêmero p/ cert cliente só funciona em runtime **Linux** (OpenSSL); no **Windows** o schannel recusa no handshake → dev local sobe com `Habilitado=false` (deploy é Debian `aspnet:10.0`).
