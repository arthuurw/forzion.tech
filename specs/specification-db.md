...
- Estrutura IDÊNTICA em todos os schemas após todas as migrations: **45 tabelas BASE** = 43 tabelas EF + `ai_token_usage` (não-EF) + `__EFMigrationsHistory` (controle). Inclui MFA e DataProtection; NÃO inclui `notas_fiscais` (dropada). Nível de migration por schema PODE divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
