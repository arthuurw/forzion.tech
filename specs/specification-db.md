...
- Schemas com estrutura IDÊNTICA: `homolog` (deploy ativo, canônico), `develop` (sandbox), `public` (sandbox/legado sincronizado). 37 tabelas BASE cada (35 entidades EF + `ai_token_usage` não-EF + `__EFMigrationsHistory` de controle) após todas as 40 migrations aplicadas (`notas_fiscais` adicionada na 40ª; `dados_fiscais_*` é owned VO em `treinadores`, não conta como tabela); paridade dos 3 schemas no nível 40 (`CriarNotasFiscaisEDadosFiscaisTreinador`, todas as 40 migrations) verificada 2026-06-17. Nível atual por schema pode divergir após novas migrations — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
...
- 40 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última `CriarNotasFiscaisEDadosFiscaisTreinador` — cria `notas_fiscais` (+1 tabela BASE) e adiciona as colunas owned `dados_fiscais_*` em `treinadores`). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
