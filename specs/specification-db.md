...
- `ai_token_usage`: telemetria FORA do domain model (sem entity/DbSet), mas criada por migration EF (`AdicionarAiTokenUsage`, raw SQL idempotente `CREATE TABLE IF NOT EXISTS`) → migrate provisiona em qualquer schema (dono = forzion_api, sem grant anon herdado). Schemas legados (homolog/develop/public) já a tinham (pré-migration) → `IF NOT EXISTS` é no-op lá.
- 58 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última por timestamp `AdicionarAiTokenUsage` (20260710003706)). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
...
- **`forzion_p` (PROD, ref `zdmgqnostkqiolutrbwq`) PROVISIONADO 2026-08-06**: `scripts/provision-forzion_p-grants.sql` → `app migrate` one-shot (`compose -p prd run --rm --no-deps backend migrate`, que aplica schema **E seed** e sai — `Program.cs:20`). Estado validado: 45 tabelas em `public` **todas owned by forzion_api** (≠ homolog-`public`, onde o dono seria postgres), 1 sequence, 59 migrations, seed 9 grupos/90 exercícios globais/5 planos (`Basic|Free|Pro` ativos, `Elite|ProPlus` inativos)/1 admin verificado, SEM `notas_fiscais`, COM `ai_token_usage`. **Zero grant de tabela p/ `anon`/`authenticated`/`service_role`** — não por REVOKE, e sim por construção: tabela criada POR `forzion_api` não herda default ACL (só existem `pg_default_acl` de `postgres`/`supabase_admin`). `data_protection_keys` nasce VAZIA — só popula no 1º boot do web host, não no migrate.
- **GOTCHA — reset de senha do Supabase DESFAZ o hardening de grants** (verificado 2026-08-06 no `forzion_p`): "Reset database password" no dashboard re-provisiona as roles gerenciadas e RESTAURA os grants default do schema (`PUBLIC`/`anon`/`authenticated`/`service_role` voltam com `USAGE`), revertendo o `REVOKE` de `0adff467`. Rodar `provision-forzion_p-grants.sql` de novo (idempotente) DEPOIS de qualquer reset. O script revoga privilégio de TABELA/SEQUENCE desses roles, NÃO o `USAGE` do schema — `anon=U`/`authenticated=U` permanecem no ACL por design; a proteção real é (a) ownership `forzion_api`, (b) zero grant de tabela, (c) Data API OFF.
- **Credencial da plataforma pode dessincronizar em projeto dormente**: `forzion_p` (criado 2026-07-02, sem uso) passou a recusar TODA query da Management API/MCP com `28P01 password authentication failed` p/ `postgres` E `supabase_read_only_user` — inclusive SQL Editor. Reset da senha do banco ressincroniza. Não afeta `DB_CONNECTION` do app se ele conecta como `forzion_api` (role própria, senha independente).
