...
- 52 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última por timestamp `RedefinicaoSenhaSegundoFatorLockout`). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
