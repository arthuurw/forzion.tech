...
- Schemas com estrutura IDÊNTICA: `homolog` (deploy ativo, canônico), `develop` (sandbox), `public` (sandbox/legado sincronizado). 42 tabelas BASE cada (40 entidades EF + `ai_token_usage` não-EF + `__EFMigrationsHistory` de controle) após todas as 42 migrations aplicadas; as migrations da feature MFA (`AdicionarTabelasMfa` +4 tabelas, `AdicionarTrocaEmailToken` +1 `troca_email_tokens`) e da feature fiscal (`CriarNotasFiscaisEDadosFiscaisTreinador` +1 `notas_fiscais`; `dados_fiscais_*` é owned VO em `treinadores`, não conta como tabela) **chegam com suas branches** — podem ainda NÃO estar aplicadas em homolog/develop/public até o deploy. Nível atual por schema pode divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
...
- 42 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última por timestamp `AdicionarTrocaEmailToken`). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
