...
- Estrutura IDÊNTICA em todos os schemas após todas as migrations: **48 tabelas BASE** = 46 tabelas EF (inclui `leads`/`lead_convites`/`lead_interacoes` do gateway de agentes fatia 2) + `ai_token_usage` (não-EF) + `__EFMigrationsHistory` (controle). Inclui MFA e DataProtection; NÃO inclui `notas_fiscais` (dropada). Nível de migration por schema PODE divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
