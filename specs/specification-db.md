...
- Schemas com estrutura IDÊNTICA: `homolog` (deploy ativo, canônico), `develop` (sandbox), `public` (sandbox/legado sincronizado). 37 tabelas BASE cada (35 entidades EF + `ai_token_usage` não-EF + `__EFMigrationsHistory` de controle) após todas as 40 migrations aplicadas (`notas_fiscais` adicionada na 40ª; `dados_fiscais_*` é owned VO em `treinadores`, não conta como tabela); paridade dos 3 schemas verificada 2026-06-16 no nível de então — as 2 últimas migrations podem não estar aplicadas nos 3 schemas. Nível atual por schema pode divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
