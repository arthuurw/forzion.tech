...
- Schemas com estrutura IDÊNTICA: `homolog` (deploy ativo, canônico), `develop` (sandbox), `public` (sandbox/legado sincronizado). 38 tabelas cada (37 EF + ai_token_usage) após todas as 36 migrations aplicadas. Nível atual por schema pode divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
...
- 36 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última `AdicionarRefreshTokens`). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
