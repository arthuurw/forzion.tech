...
- Schemas com estrutura IDÊNTICA: `homolog` (deploy ativo, canônico), `develop` (sandbox), `public` (sandbox/legado sincronizado). 41 tabelas BASE cada (39 entidades EF + `ai_token_usage` não-EF + `__EFMigrationsHistory` de controle) após todas as 41 migrations aplicadas; as 2 últimas (`AdicionarTabelasMfa` +4 tabelas MFA, `AdicionarTrocaEmailToken` +1 `troca_email_tokens`) **chegam com a feature MFA** — podem ainda NÃO estar aplicadas em homolog/develop/public até o deploy desta branch. Paridade dos 3 schemas verificada 2026-06-16 no nível de então. Nível atual por schema pode divergir — conferir antes (§APLICAÇÃO DE MIGRATIONS §GOTCHA).
...
- 41 migrations EF (arquivos não-Designer/Snapshot em `Infrastructure/Migrations/`; última `AdicionarTrocaEmailToken`). Tabela de controle `__EFMigrationsHistory` por schema (colunas snake_case: `migration_id` varchar(150) PK, `product_version` varchar(32); EF `ProductVersion` = versão do pacote em `forzion.tech.Infrastructure.csproj`, não fixar aqui).
