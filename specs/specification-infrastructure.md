...
## ENV / SECRETS
- **VM hmg** `/opt/forzion/.env` (NÃO versionado): `APP_ENV`, `DB_CONNECTION`, `DB_SCHEMA`, `JWT_SECRET`/`ISSUER`/`AUDIENCE`, `CORS_ORIGINS`, `STRIPE_SECRET_KEY`/`WEBHOOK_SECRET`/`URL_BASE`, `RESEND_API_KEY`/`WEBHOOK_SECRET`, `APP_FRONTEND_BASE_URL`, `INTERNAL_API_KEY` (chave do endpoint `/internal/processar-renovacoes`; vazio → 401), WhatsApp (opcional), `PACT_*`. Compose mapeia → `Resend__ApiKey`, `Stripe__SecretKey`, `Internal__ApiKey`, etc.
- **GitHub Actions secrets**: `HOMOLOG_HOST`/`HOMOLOG_SSH_KEY` (deploy), `INTERNAL_API_KEY` (billing-renewal; MESMO valor do `.env` da VM — comparação constant-time no endpoint).
