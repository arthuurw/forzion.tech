...
- **Workflows de billing** (4 endpoints internos, cron + `workflow_dispatch`; `curl -f POST https://${HOMOLOG_HOST}/<rota>` com header `X-Internal-Key: ${INTERNAL_API_KEY}`; `failure()` abre issue):
...
- **GitHub Actions secrets** (env `homolog`): `HOMOLOG_HOST` (= IP tailnet `100.x.y.z` da VPS, NÃO o público)/`HOMOLOG_SSH_KEY` (deploy), `TS_OAUTH_CLIENT_ID`/`TS_OAUTH_SECRET` (OAuth client Tailscale, scope Auth Keys:Write + `tag:ci`; runner-no-tailnet), `INTERNAL_API_KEY` (billing-renewal-treinador + billing-renewal + billing-reconciliation; MESMO valor do `.env` da VM — comparação constant-time no endpoint).
