...
- **Treinador*** — perfil treinador (state machine própria). `Criar(contaId, nome, agora, telefone?=null)` → `Status=AguardandoAprovacao`. Métodos: `Aprovar(aprovadoPorId)` (→Ativo, set AprovadoPorId/Em, emite `TreinadorAprovadoEvent`); `Reprovar(reprovadoPorId)` (→Inativo, emite `TreinadorReprovadoEvent`); `Inativar(inativadoPorId?)` (→Inativo, emite `TreinadorInativadoEvent`); `AtribuirPlano(planoPlataformaId, agora)` → `Result` (retorna `Result.Failure(TreinadorErrors.PlanoTreinadorInativo)` se Inativo; NÃO lança); `AtualizarNome(nome)`; `AlterarModoPagamento(novoModo, agora)` → `Result`: falha `modo_inalterado` se igual ao atual; falha `cooldown_modo_pagamento` se `< CooldownModoPagamentoDias` (=90) desde `ModoPagamentoAlunoAlteradoEm`; senão set modo + `ModoPagamentoAlunoAlteradoEm=agora` (orquestração de cancelamento/criação de assinaturas é do handler — [specification-stripe]); guards `ValidarDisponibilidade()` (lança se ≠Ativo), `ValidarParaExclusao()` (só exclui se Inativo). Inv: nome 1..100; aprovar/reprovar só de AguardandoAprovacao. Const `CooldownModoPagamentoDias=90`. Props: `ModoPagamentoAlunoAlteradoEm` (nullable); `PlanoCortesiaId` (nullable, cortesia manual concedida por admin) e `AlunosAcimaDoCapDesde` (nullable, marca início da janela de graça de 3 meses quando vínculos ativos excedem o plano EFETIVO). `DefinirCortesia(planoCortesiaId?, agora)` → `Result` (falha `TreinadorErrors.PlanoCortesiaIdInvalido` se Guid.Empty explícito; guarda anti-abuso — cortesia não pode custar MENOS que a assinatura paga ATIVA do treinador — vive na Application/`DefinirCortesiaHandler` por cruzar agregados (`AssinaturaTreinador`/`PlanoPlataforma`), falha `TreinadorErrors.CortesiaAbaixoDoPago`, NÃO reproduzida no método). `MarcarAcimaDoCap(agora)`/`LimparAcimaDoCap(agora)` — idempotentes (no-op se já setado/já null) — orquestrados pelo job `ProcessarLimiteAlunosHandler` ([specification-backend]): carimbam/limpam a janela de 3 meses de graça antes da apara automática de vínculos excedentes. Plano EFETIVO = `MAX(Preco)` entre `AssinaturaTreinador` Ativa e `PlanoCortesiaId` (`IPlanoEfetivoResolver`/`PlanoEfetivoResolver`, Application; fail-closed → Free/cap=0 se nem o plano Free estiver configurado na plataforma) — substitui `ICapacidadePlano`/`Treinador.PlanoPlataformaId` puro como fonte de `MaxAlunos` (§8). `Anonimizar(agora)` idempotente via flag PERSISTIDA `Anonimizado` (scrub nome/telefone; no-op se já anonimizado — guard sobrevive ao reload do banco, [specification-db]).
...
- **VinculoTreinadorAluno*** — relação treinador↔aluno (aprovação + pacote). `Criar(treinadorId, alunoId, agora, pacoteId?=null)` → `Status=AguardandoAprovacao`, emite `VinculoPendenteCriadoEvent`. Métodos: `Aprovar(aprovadoPorId, pacoteId)` (→Ativo, set Pacote/AprovadoPor/Em/DataInicio, emite `VinculoAprovadoEvent`); `Inativar()` (→Inativo, set DataFim, SEM evento). Inv: ids≠Empty; aprovar só de AguardandoAprovacao; pacoteId≠Empty na aprovação. Prop `PreservarNoLimite` (bool, default false). `DefinirPreservacao(preservar, agora)` → `Result` (sempre sucesso, sem invariante própria) — seta a flag; consumida pela apara de excedente do job `ProcessarLimiteAlunosHandler` ([specification-backend]): preservados têm PRIORIDADE sobre não-preservados mas NÃO furam o cap — se `preservados.Count > capEfetivo`, TODOS os não-preservados são cortados e o excedente ainda sai dos preservados mais recentes (por antiguidade); senão, o cap remanescente (`capEfetivo - preservados.Count`) fica com os não-preservados mais antigos, corta os mais recentes primeiro.
...
- **Exercicio** — global (`TreinadorId=null`) ou do treinador (`Criar`/`Atualizar` em `Domain/Entities/`). Prop derivada `IsGlobal` (= TreinadorId null). Inv: nome 1..100; grupoMuscularId≠Empty; descricao ≤500; comoExecutar ≤2000 (>2000 → `ExercicioErrors.ComoExecutarMuitoLongo`); videoUrl parseada por `YouTubeVideoId` (§3) → persiste `VideoId` (11ch) ou falha. PATCH parcial (null=mantém, string vazia=limpa) p/ descricao/comoExecutar/videoUrl — orientação reutilizável herdada na cópia global→treinador (`CopiarExercicioGlobalHandler`).
- **Treino** (aggregate root) — ficha de treino + exercícios ordenados; métodos (`Criar`/`Atualizar`/`AdicionarExercicio`/`RemoverExercicio`/`Duplicar`/`DuplicarPara`) em `Domain/Entities/Treino.cs`. Guard estático `ValidarMutabilidade(bool foiExecutado)` (lança `TreinoExecutadoException`; gateia TODA mutação de ficha já executada — incluindo `Atualizar` de cabeçalho, não só exercícios). `foiExecutado` = `IExecucaoTreinoRepository.ExisteParaTreinoComAlunoAtivoAsync` — `true` somente se existe execução E o `TreinoAluno` do executor está `Ativo`; aluno inativado/desvinculado (`TreinoAlunoStatus.Inativo`) NÃO trava → treino volta a ser mutável. Inv: nome 1..100; treinadorId≠Empty; dataFim≥dataInicio; `Atualizar` valida ANTES de mutar (validate-then-mutate, sem estado parcial). Sem eventos.
- **TreinoExercicio** (filho de Treino) — exercício na ficha + séries. Factory `internal Criar`; `AdicionarSerie`/`AtualizarSeries`(≥1 grupo, recria)/`AtualizarObservacao`(≤500)/`internal AlterarOrdem`. Inv: ids≠Empty.
- **SerieConfig** (filho de TreinoExercicio) — config de séries. Factory `internal Criar`. Inv: quantidade≥1; repeticoesMin≥1; repeticoesMax≥min; carga≥0; descanso≥0. Sem mutação.
...
- **HealthSnapshot** — snapshot diário de saúde. `Criar(ambiente, status, payloadJson, agora)`. Inv: ambiente/payload não-vazios. `MarcarEmailEnviado(sucesso)` — mutação pontual pós-persist (sem invariante, sem evento): registra se o e-mail do relatório saiu de fato (`ExecutarRelatorioSaudeHandler`/`RelatorioSaudeDiarioService`, ambos via 2º `CommitAsync` após o try/catch de envio — ordering do #320 intocado). Prop `EmailEnviado` (`bool?`; `null`=não rastreado/snapshot pré-feature). Fora isso, append-only.
...
- **TipoAcaoAprovacao** {AprovacaoTreinador, ReprovacaoTreinador, InativacaoTreinador, AprovacaoVinculo, ReprovacaoVinculo, InativacaoVinculo, AtribuicaoPlanTreinador, ExclusaoTreinador, ExportacaoDados, AnonimizacaoConta, ConsentimentoAnamnese, DefinicaoCortesiaTreinador} — tipo registrado em LogAprovacao (ExportacaoDados/AnonimizacaoConta/ConsentimentoAnamnese = trilha LGPD; ConsentimentoAnamnese = consentimento art. 11 no cadastro do aluno, observacao = versão do termo; DefinicaoCortesiaTreinador = admin concede/revoga `Treinador.PlanoCortesiaId` via `DefinirCortesiaHandler`, observacao = plano/"removida"). Persistido como text (`HasConversion<string>`) → novo valor sem migration.
...
- **TipoNotificacao** {NovoTreino, ExecucaoRegistrada, Reforco, LembreteLeve, Recuperacao, MarcoStreak, DigestTreinador, LimiteAlunosExcedido, LimiteAlunosLembrete, LimiteAlunosAplicado} — tipo do item de `Notificacao`. `NovoTreino`/`ExecucaoRegistrada` = disparados por domain event; `Reforco`/`LembreteLeve`/`Recuperacao`/`MarcoStreak` = nudges do scan de aderência do aluno; `DigestTreinador` = resumo diário ao treinador; `LimiteAlunosExcedido`/`LimiteAlunosLembrete`/`LimiteAlunosAplicado` = ciclo da graça de 3 meses de `Treinador.AlunosAcimaDoCapDesde` (`ProcessarLimiteAlunosHandler`, job): Excedido no carimbo inicial, Lembrete em T-30/T-7/T-1 dias do fim da janela, Aplicado após a apara automática de vínculos excedentes. Persistido como text (`HasConversion<string>`). Nudges Reforco/LembreteLeve/Recuperacao são MUTUAMENTE EXCLUSIVOS por dia; MarcoStreak coexiste só com Reforco (lógica em [specification-backend]).
...
| LimiteAlunosAtingido | treinador no limite de alunos do plano EFETIVO (`IPlanoEfetivoResolver.MaxAlunos`, §8) — bloqueia criação de NOVO vínculo; excedente pré-existente (ex.: downgrade) é tratado pela graça de 3 meses (`Treinador.AlunosAcimaDoCapDesde`, §2), não por esta exceção |
...
- **ICapacidadePlano** (Interfaces) — `int MaxAlunos`. Implementado por `PlanoPlataforma`. ⚠️ NÃO é mais a fonte de `MaxAlunos` usada em `LimiteAlunosAtingidoException`: `LimiteTreinadorService` (Application) valida contra o plano EFETIVO resolvido por `IPlanoEfetivoResolver` (`MAX(Preco)` entre `AssinaturaTreinador` Ativa e `Treinador.PlanoCortesiaId`, fail-closed → Free/cap=0 sem Free configurado), não contra `Treinador.PlanoPlataformaId`/`ICapacidadePlano` diretamente.
