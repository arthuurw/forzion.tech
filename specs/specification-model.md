...
- **Invariantes**: validadas na factory E nos métodos de mutação. Violação → lança `DomainException` (ou derivada). NÃO há `Result<T>` no domínio — `Result<T>` é padrão da Application ([specification-backend]). `DomainException` → HTTP **422** no `GlobalExceptionHandler` ([specification-backend]).
...
- **TimeProvider**: factories e métodos de transição de negócio recebem `DateTime agora` (injetado pela Application via `TimeProvider`) e o usam p/ `CreatedAt`/`DataInicio`/`OcorridoEm`. ⚠️ INCONSISTÊNCIA REAL: métodos de mutação mais antigos (`Pagamento.Marcar*`, `Treinador.Aprovar/Reprovar/Inativar`, `Aluno.Ativar/Inativar/Atualizar`, `Vinculo.Aprovar/Inativar`, `AssinaturaAluno.Ativar/MarcarInadimplente`, todos os `AtualizarNome`/`Atualizar`/`Inativar` de catálogo) ainda usam `DateTime.UtcNow` direto p/ `UpdatedAt` e até p/ `OcorridoEm` de eventos (ex.: `Treinador.Aprovar` emite `TreinadorAprovadoEvent(..., DateTime.UtcNow)`). Factories e os métodos do agregado de billing (`Cancelar`, `RegistrarPagamentoFalho`, `MarcarInadimplentePorDisputa`, `RegistrarPagamentoRegularizado`, `AgendarProximaCobranca`, `Conta.MarcarEmailVerificado`) já recebem `agora`. Padronizar p/ `agora` em todo lugar é refactor pendente.
