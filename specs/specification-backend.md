...
- `AppDbContext` schema-agnostic (sem `HasDefaultSchema`; schema vem do `Search Path` da connection), `UseSnakeCaseNamingConvention`, `MigrationsHistoryTable("__EFMigrationsHistory")`. Configs por entidade em `Persistence/Configurations` (`ApplyConfigurationsFromAssembly`). 30 DbSets (28 `public` + 2 `internal` `TreinoExercicio`/`ExecucaoExercicio`, composição); `SerieConfig` é mapeada SEM DbSet (acessada por navegação) ⇒ 31 tabelas EF; +`ai_token_usage` NON-EF = 32 ([specification-db]). Contagem ancorada por `Tests/Architecture/SpecInventoryTests` (quebra força atualizar esta spec). Cross-ref [specification-db] (schema/migrations/enums).
