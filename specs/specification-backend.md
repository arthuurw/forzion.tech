...
- **Vinculos**: `AprovarVinculo` (tx serializable; **exige onboarding Stripe do treinador — `ContaRecebimento.OnboardingCompleto`; senão falha `treinador_sem_onboarding` ANTES de qualquer efeito**, evitando aluno ativo sem billing; troca de treinador inativa vínculo anterior + cascateia/copia fichas via `TrarFichas`; valida limite de plano; notifica WhatsApp), `DesvincularAluno`, `ReativarVinculo`, `SolicitarTrocaTreinador`, `ListarVinculos`, `ObterVinculoAluno`.
