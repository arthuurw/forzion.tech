...
- **[GAP] Environments sem protection rule** — os GitHub Environments `production` e `homolog` têm ZERO regras (sem required reviewer, sem deployment branch policy). Um workflow disparado por push de QUALQUER branch pode ler os secrets do environment (`PROD_SSH_KEY`, `INTERNAL_API_KEY`, `TS_OAUTH_*`, `VM_TAILNET_IP`) — repo PÚBLICO, sem restrição de qual branch pode invocar `environment: production`.
