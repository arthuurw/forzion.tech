...
## CONTEXTO
Solo dev. 1 alteração por branch. Multi-folder: cada frente vive em pasta isolada (clone independente OU worktree). Prioriza baixa fricção sync local↔origin. `homolog` SEM branch protection (push direto permitido — decisão consciente, ver `secprod-supply-chain-gates` GATE-05 Out of Scope, usuário recusou perder push direto 2026-08-09). `main` TEM branch protection (hardened 2026-08-09, ver `.specs/features/secprod-supply-chain-gates`): required checks `Gate` + `Semgrep scan` (strict), PR obrigatório, `required_approving_review_count: 0`, sem force-push/delete, **`enforce_admins: true`** — nem o owner/admin pula os checks, push direto em `main` é rejeitado por qualquer usuário. Repo é PÚBLICO.
...
- **GOTCHA squash-merge + promoção `homolog`→`main` relinta histórico já mergeado** (achado 2026-08-10, PR #393): job `Commitlint (PR)` roda `commitlint --from origin/$BASE_REF --to HEAD`; numa PR de promoção, `BASE_REF=main` e o range cobre TODOS os commits acumulados em `homolog` desde a última promoção — incl. squash-commits já validados e deployados. GitHub anexa ` (#NNN)` ao título do PR automaticamente no squash-merge, DEPOIS do commitlint já ter validado a mensagem pré-squash; se o header original já estava perto do limite de 100, o sufixo pode estourar — e como o squash só existe pós-merge, nenhum commitlint anterior pegou isso, só a promoção pra `main` (primeira vez que o range é relintado). Reescrever o commit já mergeado em `homolog` (branch compartilhada, deployada) é descartado. Fix: `commitlint.config.mjs` ganha `ignores` que pula SÓ headers que estourariam >100 exclusivamente por causa do sufixo `(#NNN)` do GitHub (i.e., sem o sufixo, ≤100) — headers genuinamente longos continuam reprovando.
...
## EDGE CASES
- **Branch protection em `main`**: bloqueia push direto de QUALQUER usuário, incl. admin (`enforce_admins:true`) — exige PR + checks `Gate`+`Semgrep scan` verdes. Usar `gh pr create` em vez de push manual; não há bypass de emergência (se `enforce_admins` for desativado temporariamente por incidente, registrar em `STATE.md`, per `secprod-supply-chain-gates`). `homolog` segue SEM proteção alguma (push direto é o fluxo normal descrito em §PUSH/PR).
- **Environments com deployment branch policy** (2026-08-09) — `production` restrito a `main` (único branch que dispara `deploy-prod.yml`/crons em modo `production`); `homolog` restrito a `homolog`+`main` (`main` incluído porque os crons via `schedule` sempre rodam do branch default, mesmo pro item `homolog` da matrix `CRON_ENVIRONMENTS` — sem isso o cron quebraria por policy). Sem required reviewer (repo solo — reviewer só criaria auto-aprovação cerimonial). Detalhe: `.specs/features/secprod-supply-chain-gates`.
