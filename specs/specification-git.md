...
## CONTEXTO
Solo dev. 1 alteração por branch. Multi-folder: cada frente vive em pasta isolada (clone independente OU worktree). Prioriza baixa fricção sync local↔origin. `homolog` SEM branch protection (push direto permitido). `main` TEM branch protection PARCIAL (auditoria 2026-08-06): required check `Gate` (strict), PR obrigatório, `required_approving_review_count: 0`, sem force-push/delete. **[GAP]** `enforce_admins: false` — o owner/admin ainda empurra direto pra `main` pulando o `Gate` inteiro. Repo é PÚBLICO.
...
## EDGE CASES
- **Branch protection em `main`**: bloqueia push direto de não-admin (exige PR + check `Gate`) — usar `gh pr create` em vez de push manual. `homolog` segue SEM proteção alguma (push direto é o fluxo normal descrito em §PUSH/PR). Ver §CONTEXTO p/ o gap `enforce_admins:false`.
