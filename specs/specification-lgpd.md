...
## DB / PII
- PII por tabela e escopo de scrub → ver `.specs/features/lgpd/spec.md` + [specification-db]. Sensível (saúde): anamnese do aluno. Retidos anonimizados: pagamentos, assinaturas_aluno, logs_aprovacao, treinos/execuções (ids). `mensagens_suporte`, `refresh_token_families`/`refresh_tokens` E as tabelas MFA (`conta_mfa`, `mfa_recovery_codes`, `mfa_challenges`, `trusted_devices`) são DELETADAS (não retidas) — texto livre / credencial de sessão/2FA sem valor fiscal.
