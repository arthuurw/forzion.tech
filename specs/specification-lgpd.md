...
5. Scrub recipient dos delivery logs: `IEmailDeliveryLogRepository.AnonimizarPorEmailAsync` + `IWhatsAppDeliveryLogRepository.AnonimizarPorTelefoneAsync` (recipient → placeholder; payload mantido p/ auditoria). `IMensagemSuporteRepository.ExcluirPorContaIdAsync` (ExecuteDelete): mensagens de suporte são texto livre (assunto/descrição) = PII potencial e sem valor fiscal → **DELETE** (não scrub), all-or-nothing na mesma transação. `IRefreshTokenFamilyRepository.ExcluirPorContaIdAsync` (ExecuteDelete, cascade nos `refresh_tokens`): purga as sessões de refresh do titular (`Rotulo` = device/user-agent = PII potencial) — também **DELETE**, mesma transação.
