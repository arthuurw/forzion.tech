...
## DB / PII
- PII por tabela e escopo de scrub → ver `.specs/features/lgpd/spec.md` + [specification-db]. Sensível (saúde): anamnese do aluno. Retidos anonimizados: pagamentos, assinaturas_aluno, logs_aprovacao, treinos/execuções (ids). `mensagens_suporte`, `refresh_token_families`/`refresh_tokens`, `password_reset_tokens` E as tabelas MFA (`conta_mfa`, `mfa_recovery_codes`, `mfa_challenges`, `trusted_devices`) são DELETADAS (não retidas) — texto livre / credencial de sessão/2FA/reset sem valor fiscal.
- **Guarda fiscal NFS-e**: `notas_fiscais` (CPF/CNPJ/razão social do tomador no DPS emitido) E os dados fiscais owned em `treinadores` (`dados_fiscais_*`) são RETIDOS — `Treinador.Anonimizar` scrub-a só nome+telefone, NÃO os dados fiscais (obrigação fiscal BR ~5 anos > apagamento; exceção legítima art. 7º LGPD). NÃO "corrigir" o `Anonimizar` p/ limpar dados fiscais. Canônico em [specification-fiscal].
...
- **Billing/transacional ISENTO** (art. 7º/8º — obrigação contratual, não consentimento): cobrança, renovação, disputa, NFS-e NUNCA consultam a flag. O opt-out não pode silenciar comunicação obrigatória de relação de consumo. In-app sempre chega (custo zero, dentro do produto). Regressão barrada por teste (`CobrancaProximaEmailAlunoOptOutTests` — billing envia mesmo com opt-out ON).
