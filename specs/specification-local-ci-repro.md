...
- **Broker Pact divide VM/nginx com o app → race com o deploy (CANÔNICO)**: o broker self-hosted (`https://pact.homologacao.forzion.tech`, containers `pact-broker`+`pact-postgres`) vive no MESMO `docker-compose.homolog.yml`/nginx do app. nginx é o ÚNICO listener 443 p/ app E broker, e `depends_on: frontend`. O deploy (`ci.yml deploy-homolog`, SSH na VM) faz `docker compose build` (builda .NET+Next NA VM — satura CPU/IO) → recreate (recria nginx via dependent) → `restart nginx`. Durante essa janela a 443 fica inalcançável por **dezenas de min** (SYN dropado → connect-timeout, não "refused"). Qualquer job que bata no broker em PARALELO ao deploy falha por RACE (falso-vermelho), não por quebra de contrato.
  - **FIX ESTRUTURAL (2026-06-16)**: os steps que tocam o broker rodam DEPOIS do deploy, ordenados por `needs:` no `ci.yml`: `deploy-homolog` → `pact-publish` (consumer: gera+publica+can-i-deploy) → `pact-provider-verify` (provider: `dotnet test`). Broker estável. `contract.yml` virou gate de PR puro (gera+valida consumer + provider file-source SEM broker; trigger só `pull_request`/`dispatch`). `pact-provider.yml` ficou só `workflow_dispatch`+cron terça (re-verify standalone, NÃO corre deploy). `workflow_run` NÃO serve p/ ordenar cross-workflow: lê o workflow do branch default `main`, onde os arquivos não existem → por isso a consolidação no `ci.yml` (mesmo workflow = `needs:` resolve).
  - `needs` dos jobs novos usa `if: always() && needs.<x>.result == 'success'` — o `success()` IMPLÍCITO avalia o grafo TRANSITIVO; sem isso os jobs PR-only skipados em push fariam pular também (mesmo gotcha do `deploy-homolog`).
  - **Rede de segurança (não o fix)**: o step "Aguarda broker disponivel" faz `curl` heartbeat com `--connect-timeout 10 --max-time 20` (12×sleep 15) → falha rápido em vez de pendurar. HISTÓRICO: antes do hardening o curl sem timeout pendurava 136s/tentativa → run de 30min (ex. push pós-PR#163, broker DOWN). Cobre só restart breve; NÃO cobria a janela de deploy (daí o fix estrutural).
  - **Broker genuinamente DOWN** (não-race): app `homologacao.forzion.tech`=200 + broker heartbeat autenticado pendura/timeout → broker interno degradado. App NÃO afetado (DB do app separado do `pact-postgres`; `can-i-deploy` é `continue-on-error`). Remediação (VM): `docker compose -f docker-compose.homolog.yml ps` / `restart pact-broker pact-postgres`.
