...
- **`forzion.tech.PactVerification` FORA da solution — drift latente (CANÔNICO, recorrente)**: o projeto de verificação do PROVIDER (Pact, backend .NET) **NÃO** está em `forzion.tech.slnx`, então `dotnet build forzion.tech.slnx` (gate do PR + "build completo" do AGENTS FLUXO) **NÃO o compila**. Mudanças na API de domínio quebram esse projeto **SILENCIOSAMENTE** — só estoura no PUSH em homolog (`pact-provider.yml` é o único que o builda → "pact falhou em homolog"). Gatilhos típicos: refactor `Result<T>` (factories `Criar`/`Email.Criar` passam a retornar `Result<T>` → falta `.Value`); mudança de assinatura de entidade (ex.: `VinculoTreinadorAluno.Aprovar(+agora)`). **Remediação OBRIGATÓRIA**: ao mexer em `forzion.tech.Domain` (factories, `Result<T>`, assinaturas) ou nas responses/handlers que o provider mocka (`ForzionApiProviderTests.cs`), buildar EXPLÍCITO antes do PR: `dotnet build forzion.tech.PactVerification/forzion.tech.PactVerification.csproj -c Release`. Achado 2026-06-06: refactor `Result<T>` deixou ~8 erros de compilação no provider (`.Value` ausente em `Conta/Aluno/Treinador/Treino/TreinoAluno/Vinculo.Criar` + `Email.Criar` + `Aprovar` com arg faltando) que vazaram p/ homolog.
