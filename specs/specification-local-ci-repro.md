...
| backend integração (Testcontainers) | `dotnet test forzion.tech.Tests --filter "Category=Integration"` | ✅ (Docker) | contagem: ver [specification-tests] §4 (canônico; re-detectar via `dotnet test`); sobe Postgres efêmero. |
...
| openapi-drift | `bash scripts/gen-openapi.sh docs/api/openapi.v1.json && git diff --exit-code docs/api/openapi.v1.json` | ✅ | gera offline (sem Kestrel). Drift = regenerar+commitar. **Também roda no pre-commit local** (gate backend, após o build — `git diff --quiet`): drift do spec é pego ANTES do push, não só no CI. Ver [specification-git] §PRE-COMMIT HOOK. |
...
- **Broker Pact: runner alcança SÓ via tailnet (CANÔNICO / INVARIANTE — §Pact broker rota)**: o broker self-hosted (`https://pact.homologacao.forzion.tech`, containers `pact-broker`+`pact-postgres`) vive na MESMA VM do app (`docker-compose.homolog.yml`, atrás do nginx 443). O runner do GitHub NÃO atravessa a borda pública até o broker: a borda Hostinger DROPA o IP rotativo do runner (SYN dropado → `curl (28)` connect-timeout, não "refused") — MESMA patologia que força o `deploy-homolog` a falar com a VM via tailnet (IP privado `VM_TAILNET_IP`), não pela 22/443 pública. Sintoma: job Pact pendura em "Aguarda broker disponivel" 12×, exit 1, com o broker NO AR (heartbeat de fora da borda responde 401).
  ...
  - **`smoke.yml` cobre o MESMO drop (app edge, não broker — descoberto 2026-07-02)**: o job `Smoke @smoke` bate no app `https://homologacao.forzion.tech` (readiness `/api/backend/health/ready` + Playwright) DIRETO do runner pela borda pública — a última rota runner→VM que ainda NÃO usava tailnet. MESMA patologia: a borda Hostinger dropa o IP do runner → `curl` retorna `000` (connect fail total) nas 24 tentativas de readiness → smoke reprova com homolog SAUDÁVEL (app=200 de fora da borda; Pact via tailnet=success 90s antes). DISCRIMINADOR: `000`/`curl (28)` = L4/TLS connect fail (rota), NÃO `502/503` (app caído). FIX: `smoke.yml` entra no tailnet (`tag:ci`) + resolve o host homolog → `VM_TAILNET_IP` em `/etc/hosts` ANTES do readiness; `curl`/undici (`apiRequestContext`) e browsers Playwright honram `/etc/hosts` via resolver OS; TLS/SNI intactos (Host preservado). Reusa o grant ACL `tcp:443` já existente. Sintoma histórico: runs `28629355010`/`28629182250` (readiness `000000` 120s).
  - A ordem pós-deploy `deploy-homolog → pact-publish → pact-provider-verify` é só ordem de PUBLICAÇÃO consumer→provider (NÃO anti-race; a causa da falha era ROTA, não timing).
