...
- **[GAP] Não repassa `X-Forwarded-For`/`X-Real-IP` ao backend.** O `fetch` ao `API_BASE` não injeta esses headers ⇒ o backend (`RateLimitPartitionKeys.KeyFromIp` sobre `ctx.Connection.RemoteIpAddress`) enxerga sempre o IP do container `frontend`, nunca o IP real do cliente. Isso colapsa a partição por IP do rate-limit do backend (`auth`, `internal`, `read`/`write` anônimo) num balde global por rota — efetivamente um limite único pra plataforma inteira nesse caminho. Só `/webhooks/*` (que vai direto ao backend pelo nginx, fora do BFF) preserva o IP real. Mesmo gap em `src/app/api/auth/*` (ver §API ROUTES DE AUTH). Cross-ref [specification-security] §4.
- **[GAP] `/internal/*` alcançável pela borda via este proxy.** `GET /api/backend/internal/...` chega ao backend (401 — falta de `X-Internal-Key`, nunca 404) em vez de ser barrado antes; o bloqueio do nginx cobre só o caminho direto (`/internal/` → 404). Não explorável (allowlist de headers acima nunca repassa `X-Internal-Key`), mas o endpoint interno é alcançável pela internet.
