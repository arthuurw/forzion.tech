...
- **`Sobre`** (claro, `_landing/Sobre.tsx`, feature `landing-sobre`): depois de `Faq` (claro), última seção antes do rodapé — reposicionado (2026-08-10, ajuste visual pós-deploy) do slot original entre `PlanosSlab` e `Faq`; ambas as posições respeitam a não-adjacência de preto (`Sobre` nunca é vizinha de `PlanosSlab`). Fundo `background.paper`, `py:{xs:6,md:8}`, título `h4` sem eyebrow, com "forzion"/".tech" em cores separadas (`brand.label`/`text.primary`) via componente `Forzion` (`sobre/content.tsx`) — mesmo padrão do wordmark em `Diferenciais.tsx` (NÃO o de `Logo.tsx`, que usa `primary.main` e só serve pra fundo escuro). CTA `Button variant="contained" color="primary"` → `/sobre` (amarelo com `contrastText` escuro — botão preenchido não viola a regra de contraste em fundo claro, que trata de TEXTO/ícone diretamente sobre a superfície, não de um botão com fundo próprio). `/sobre` é o nó do conjunto institucional: linka `/privacidade` e `/acessibilidade`, e é o 3º link do rodapé da landing. Conteúdo institucional (razão social, CNPJ, propósito) vive só no código de `sobre/page.tsx`/`sobre/content.tsx` (`Forzion`/`PorQueExistimos`, compartilhados com a seção da landing) + `.specs/features/landing-sobre/context.md` — não repetido aqui, para não criar 2ª fonte da verdade. Título de `/sobre` (`PageHeader`) também usa `Forzion` — exige `PageHeader.title: ReactNode` (widened de `string`, retrocompatível).
