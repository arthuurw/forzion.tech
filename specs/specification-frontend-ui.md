...
- **`Sobre`** (claro, `_landing/Sobre.tsx`, feature `landing-sobre`): entre `PlanosSlab` (preto) e `Faq` (claro) — narrativa produto→preço→confiança institucional→dúvida, e mantém a não-adjacência de preto (isola `Planos` de `Faq`). Fundo `background.paper`, `py:{xs:6,md:8}`, título `h4` sem eyebrow. CTA `Button variant="outlined" color="secondary"` → `/sobre` (amarelo/`primary.main` não usado — regra de contraste em fundo claro). `/sobre` é o nó do conjunto institucional: linka `/privacidade` e `/acessibilidade`, e é o 3º link do rodapé da landing. Conteúdo institucional (razão social, CNPJ, propósito) vive só no código de `sobre/page.tsx` + `.specs/features/landing-sobre/context.md` — não repetido aqui, para não criar 2ª fonte da verdade.
