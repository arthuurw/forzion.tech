...
## 1. O QUE EXISTE HOJE
Fatia 0 (`agents-f0-hmac`), fatia 1 (`agents-f1-perfil-publico`), fatia 2 (`agents-f2-leads`) e fatia 3 (`agents-f3-agenda`) entregues POR COMPLETO: grupo de rota + verificação de assinatura + envelope de erro + rate limit + `GET /health` + `GET business-info` + `GET services` (§6-A) + `POST leads` (§6-B, a única escrita da borda de agente) + `GET availability` (§6-C). A fatia 2 vai além da borda: esteira autenticada do treinador (`/treinador/leads/*` — lista/filtros/detalhe/histórico/transição de status/interação/métricas), notificação in-app+e-mail de lead novo, conversão lead→aluno por convite tokenizado (`/auth/convite/{token}` público + `POST /auth/register/aluno` com `conviteToken`), expurgo LGPD automático (180d sem toque) e atendimento admin ao titular sem conta (`/admin/leads`). A fatia 3 também vai além da borda: `Treinador.FusoHorario`/`PoliticaAgenda`, agregado `BloqueioAgenda` e as 6 rotas JWT do treinador em `/treinador/agenda/*` + fuso em `/treinador/perfil-publico`. Esse restante não é documentado NESTE arquivo, que cobre só a borda `/internal/agents/v1` — canônico em [specification-lgpd] §LEADS, [specification-db] §Leads/§Treinadores,Pacotes,Vínculos e [specification-security]. O caminho restante do contrato (`POST booking-requests`) **não existe** (404 de rota, sem stub, sem 501) — é a fatia 4 (§8).
...
| `service_not_found` | 404 | pacote inexistente, de outro treinador, não público ou sem `DuracaoMinutos`, emitido por `GET availability` (fatia 3); reservado também para `POST booking-requests` (fatia 4). **Não emitido pela fatia 1** — `GET services` lista, nunca resolve um item, então não há 404 por serviço ali |
| `slot_not_found` | 404 | slot derivado que nunca existiu (fatia 4 — `POST booking-requests` referencia um `slotId` que a consulta não devolveria) |
...
| `dependency_unavailable` | 503 | `agents-ready` `Unhealthy`; falha do repositório ou fuso persistido que não resolve durante `GET availability` (fatia 3, §6-C) — **nunca** `200 []` |
...
**`capacityRemaining`** = `Pacote.CapacidadeMaxima` CHEIO — nada reserva nesta fatia (`POST booking-requests` não existe ainda). A fatia 4 subtrai as confirmações.
...
**Achado pré-existente, registrado para a FATIA 4 (não resolvido aqui — conformidade deliberada, não bug desta fatia)**: `PacoteRepository.ListarPublicosPorTreinadorAsync` (`PacoteRepository.cs:39-43`) filtra `IsPublico` mas **não** `IsAtivo`. `GET availability` conforma ao MESMO filtro de `GET services` (não diverge — pacote inativo já aparece hoje em `GET services`); a pergunta "pacote inativo pode ser agendado?" só materializa risco real quando houver reserva. Registrado em `.specs/STATE.md` AD-019.
