...
- **`Request.Path` + `Request.QueryString` vs request-target cru.** `HttpRequest.Path` é o caminho DECODIFICADO pelo Kestrel; `QueryString.Value` é cru. Aceito porque os 6 caminhos do contrato só têm `tenantId` (UUID) como path param — nunca texto livre em segmento de caminho; texto livre só entra em query (já cru) e corpo, onde decodificado ≡ cru. `IHttpRequestFeature.RawTarget` foi descartado por não haver confirmação empírica de que `TestServer` o popula (suíte verde contra comportamento diferente do de produção é pior que o gotcha). **Reavaliar se alguma fatia futura introduzir path param não-UUID.**
