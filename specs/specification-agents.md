...
**`capacityRemaining`** = `Pacote.CapacidadeMaxima` MENOS as confirmadas ([specification-db] §Solicitações de Agendamento) cujo `[InicioUtc,FimUtc)` INTERSECTA o slot — nunca por igualdade de `slotId` (R2 do design da fatia 4, ver AD-020). Uma única query (`ListarConfirmadasNoIntervaloAsync`) traz todas as confirmadas do intervalo consultado; o abatimento por slot é feito em memória — nunca uma query por slot. Slot com `capacityRemaining <= 0` é OMITIDO da resposta (não marcado, D-F) — mesma filosofia de bloqueio, que também some em vez de aparecer indisponível. Regressão: `ConsultarDisponibilidadeCapacidadeTests` (inclui o teste que mataria uma implementação por igualdade de `slotId`: confirmar, mudar `Pacote.DuracaoMinutos`, a capacidade continua abatida mesmo com `slotId` novo).
...
**`slotId` validado por RE-DERIVAÇÃO, nunca aceito do cliente** — mesmo princípio de `GET availability` (§6-C): re-deriva `[agora,agora+HorizonteDias]` para o serviço e procura o hash. Slot que deixou de existir (bloqueio criado depois da consulta, política encurtada, fora do horizonte, já passou) vira `404 slot_not_found` de graça, sem regra duplicada.
...
| Rota | Handler | Nota |
|---|---|---|
| `GET /treinador/agenda/solicitacoes` | `ListarSolicitacoesHandler` | filtro opcional por status, paginado, projeta os dados do lead (placeholder quando anonimizado) |
| `POST .../{id}/confirmar` | `ConfirmarSolicitacaoHandler` | tx `Serializable`+retry ([specification-concurrency] §3) — único ato que consome capacidade |
