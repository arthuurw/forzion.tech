...
- **Pact — dois caminhos de verificação do provider** (shift-left):
  ...
  - **homolog (broker)**: `pact-provider.yml` — método `VerificaContratosPublicadosNoBroker` (`WithPactBrokerSource` + publish + can-i-deploy). Guard de **heartbeat-wait** antes do verify (loop 12×/retry 15s) → mata falso-vermelho por race deploy↔broker (broker mora na VM homolog, deploy reinicia nginx no mesmo push).
