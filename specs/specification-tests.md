...
NOTA DE PROCESSO (agente ≠ humano): a disciplina é a QUALIDADE do teste acima, NÃO o ritual red→green→refactor iteração-a-iteração. Com o agente escrevendo código+teste juntos, o valor do TDD ("o teste falharia sem o código") se obtém por design — escrever asserção de comportamento real (não tautologia) e confirmar suíte verde no fim. Cerimônia de rodar-falhar-rodar a cada passo é desperdício de token sem ganho. INTOCÁVEL permanece: §1.3 (teste que falha não é enfraquecido/skipado pra passar) e §4 (contagem não regride).

ENFORCEMENT HONESTO: cobertura por piso (§8) só impede a suíte sumir; qualidade de asserção é mutation (Stryker §7/§8) + revisão de diff no PR (teste novo é lido). Sem gate hard p/ AAA/naming/over-mock → checklist de self-review (DoD em AGENTS.md).
