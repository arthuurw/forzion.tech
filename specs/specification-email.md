...
- **`IEmailBackgroundDispatcher` — envio fora do request-path** (`EmailBackgroundDispatcher`, singleton). `Disparar(Func<IEmailService,CancellationToken,Task>)` NÃO faz mais fire-and-forget `Task.Run` solto (perdia envios no SIGTERM). Agora: `Channel` bounded (capacidade 500, `FullMode=Wait`) + worker único (`Task.Run` + `WaitToReadAsync`/`ReadAllAsync`); cada item resolve `IEmailService` de um `scope` DI NOVO (`IServiceScopeFactory`) e roda com o CT interno (cancelado só no fim do dreno). Overflow → `TryWrite` retorna `false` → `DropsContados++` + `LogWarning` (nunca silencioso; mesmo gotcha `Wait` vs `Drop*` do sink de erro, [specification-observability §1]). **Drain no shutdown** (PERF-06): `RegistrarDrenoNoShutdown(IHostApplicationLifetime)` é chamado pelo `EmailBackgroundDrenoService` (`IHostedService`, registrado em `Api/Extensions/DependencyInjectionExtensions`) — `ApplicationStopping` → `Writer.TryComplete()` + `Wait(5s)` esgota os envios em voo, depois cancela o CT. Exceção do envio → engolida + `LogError` (best-effort). Quem usa: OTP MFA, troca de e-mail, verificação, esqueci-senha.
