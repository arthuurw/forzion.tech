...
-- do que cria e herda todos os privilégios. Os GRANT/ALTER DEFAULT abaixo cobrem objetos criados
-- por OUTRO role (ex.: public.ai_token_usage via CREATE TABLE ... LIKE) e blindam o pg_dump do
-- backup contra "permission denied for sequence" (incidentes #260/#271).
...
-- 4. Objetos FUTUROS criados por postgres (ex.: ai_token_usage). ALTER DEFAULT PRIVILEGES é por
--    role criador — nomear postgres garante que grants futuros apliquem a forzion_api.
