...
public sealed class VinculoAprovadoCriarAssinaturaHandler(
    ...
    ILogger<VinculoAprovadoCriarAssinaturaHandler> logger) : IDomainEventHandler<VinculoAprovadoEvent>
{
    public async Task HandleAsync(VinculoAprovadoEvent domainEvent, CancellationToken cancellationToken = default)
    {
        ...
        if (pacote is null)
        {
            logger.LogWarning("Pacote {PacoteId} não encontrado — assinatura não criada.", vinculo.PacoteAlunoId);
            return;
        }

        var assinatura = Domain.Entities.Assinatura.Criar(
            vinculo.Id,
            pacote.Id,
        ...
        logger.LogInformation("Assinatura {AssinaturaId} criada via evento para vínculo {VinculoId}.",
